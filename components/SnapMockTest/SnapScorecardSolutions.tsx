...
import { SnapUserAnswers } from './SnapExamInterface';
import { GenericStudentInfo } from '@/components/GenericMockTest/GenericRegistrationForm';

interface SnapScorecardSolutionsProps {
  student: GenericStudentInfo;
  answers: SnapUserAnswers;
  onReset: () => void;
}

export function SnapScorecardSolutions({ student, answers, onReset }: SnapScorecardSolutionsProps) {
  const [sectionFilter, setSectionFilter] = useState<'all' | 'english' | 'quant' | 'logic'>('all');
  const [statusFilter, setStatusFilter] = useState<'all' | 'correct' | 'incorrect' | 'skipped'>('all');

  // Compute detailed scores: SNAP marking (+1 correct, -0.25 wrong out of 60 marks)
  const analytics = useMemo(() => {
    ...
    const sections: Record<'english' | 'quant' | 'logic', {
      total: number;
      attempted: number;
      correct: number;
      incorrect: number;
      ...
      english: { total: 15, attempted: 0, correct: 0, incorrect: 0, skipped: 0, score: 0, maxScore: 15, accuracy: 0 },
      quant: { total: 20, attempted: 0, correct: 0, incorrect: 0, skipped: 0, score: 0, maxScore: 20, accuracy: 0 },
      logic: { total: 25, attempted: 0, correct: 0, incorrect: 0, skipped: 0, score: 0, maxScore: 25, accuracy: 0 }
    };
    ...
    (['english', 'quant', 'logic'] as const).forEach(secKey => {
    ...
    // Projected Percentile based on totalScore (out of 60)
    let overallPercentile = '75.0';
    if (totalScore >= 45) overallPercentile = '99.5';
    else if (totalScore >= 42) overallPercentile = '98.5';
    else if (totalScore >= 39) overallPercentile = '96.0';
    else if (totalScore >= 35) overallPercentile = '91.0';
    else if (totalScore >= 30) overallPercentile = '83.0';
    else if (totalScore >= 25) overallPercentile = '72.0';
    else if (totalScore >= 20) overallPercentile = '60.0';
    else overallPercentile = '45.0';
  ...
  const filteredQuestions = useMemo(() => {
    return SNAP_MOCK_TEST_60.filter(q => {
      // Section filter
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700 max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="bg-[#2e1065] text-white rounded-[3rem] p-8 md:p-14 shadow-2xl border-4 border-[#3b0764] relative overflow-hidden">
        {/* Background watermark */}
        <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none">
          <Trophy className="w-96 h-96 -rotate-12 text-purple-400" />
        </div>

        <div className="relative z-10 flex flex-col items-center text-center space-y-8">
          <div className="inline-flex items-center gap-2 bg-purple-500/10 border border-purple-500/30 text-purple-300 px-6 py-2 rounded-full font-black text-xs uppercase tracking-[0.25em]">
            <Award className="w-4 h-4" /> Official SNAP Performance Scorecard & SIBM Predictor
          </div>

          <div>
            <p className="text-purple-200/70 text-xs md:text-sm uppercase tracking-widest font-extrabold mb-2">
              SNAP Total Score Achieved
            </p>
            <div className="flex items-baseline justify-center gap-2">
              <span className={`text-6xl md:text-8xl font-black tracking-tight ${
                analytics.totalScore >= 42 ? 'text-emerald-400' : analytics.totalScore >= 35 ? 'text-amber-400' : 'text-rose-400'
              }`}>
                {analytics.totalScore}
              </span>
              <span className="text-purple-300/40 text-3xl md:text-5xl font-black">/ 60</span>
            </div>
            <p className="text-xs text-purple-200/60 font-semibold mt-2">
              Marking Scheme: <strong className="text-white">+1.00 for Correct, -0.25 for Incorrect</strong>
            </p>
          </div>

          {/* Metrics Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full max-w-3xl">
            <div className="bg-[#3b0764]/80 p-4 rounded-2xl border border-purple-700/60 flex flex-col items-center">
              <p className="text-[10px] font-black uppercase text-purple-200/70 tracking-wider mb-1">Projected %ile</p>
              <p className="text-2xl md:text-3xl font-black text-purple-300">{analytics.overallPercentile}%</p>
            </div>
            <div className="bg-[#3b0764]/80 p-4 rounded-2xl border border-purple-700/60 flex flex-col items-center">
              <p className="text-[10px] font-black uppercase text-purple-200/70 tracking-wider mb-1">Accuracy Rate</p>
              <p className="text-2xl md:text-3xl font-black text-emerald-400">{analytics.overallAccuracy}%</p>
            </div>
            <div className="bg-[#3b0764]/80 p-4 rounded-2xl border border-purple-700/60 flex flex-col items-center">
              <p className="text-[10px] font-black uppercase text-purple-200/70 tracking-wider mb-1">Attempted</p>
              <p className="text-2xl md:text-3xl font-black text-white">{analytics.totalAttempted} <span className="text-sm font-normal text-purple-200/60">/ 60</span></p>
            </div>
            <div className="bg-[#3b0764]/80 p-4 rounded-2xl border border-purple-700/60 flex flex-col items-center">
              <p className="text-[10px] font-black uppercase text-purple-200/70 tracking-wider mb-1">Correct / Wrong</p>
          ...
          <p className="text-purple-100 font-medium text-base md:text-lg max-w-2xl leading-relaxed">
            Candidate <strong className="text-white">{student.name}</strong>, you scored <strong className="text-purple-300">{analytics.totalScore} Marks</strong> with an estimated <strong className="text-emerald-400">{analytics.overallPercentile} percentile</strong> in this full-length SNAP simulation.
          </p>
          ...
          <div className="pt-2 flex flex-wrap justify-center gap-4">
            <button
              onClick={() => {
                const text = `I scored ${analytics.totalScore}/60 (${analytics.overallPercentile}%ile) in the SNAP Official Mock Test on CareerWithMohit! 🎯 Test your SIBM Pune chances here: https://careerwithmohit.online/tools/mock-test/snap/`;
            ...
            <button
              onClick={() => {
                const url = `https://careerwithmohit.online/tools/mock-test/snap/`;
            ...
            <button
              ...
              className="bg-[#3b0764] hover:bg-[#581c87] text-purple-100 border border-purple-700/60 px-6 py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-2 shadow-md transition-all active:scale-95"
            >
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-2">
            <Target className="w-6 h-6 text-purple-600" /> Section-by-Section Score Analysis
          </h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {(['english', 'quant', 'logic'] as const).map((secKey) => {
            ...
            const meta = {
              english: { title: 'Section I: General English', sub: 'RC, Vocabulary & Grammar', maxMarks: 15 },
              quant: { title: 'Section II: Quant, DI & DS', sub: 'Arithmetic, Algebra, DI & DS', maxMarks: 20 },
              logic: { title: 'Section III: Logical Reasoning', sub: 'Analytical & Logical Reasoning', maxMarks: 25 }
            }[secKey];
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="flex items-baseline gap-2 pt-2">
                    ...
                    <span className="text-slate-400 font-bold text-lg">/ {meta.maxMarks} M</span>
                  </div>
      ...
      {/* 3. SIBM PUNE & SYMBIOSIS B-SCHOOL CUTOFF PREDICTOR */}
      <div className="bg-[#2e1065] text-white rounded-3xl p-8 border-4 border-[#3b0764] shadow-xl space-y-6">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-purple-500/20 rounded-2xl border border-purple-500/30">
            <GraduationCap className="w-6 h-6 text-purple-300" />
          </div>
          <div>
            <h4 className="text-xl font-black uppercase tracking-tight text-white">SIBM Pune & Symbiosis Cutoff Predictor</h4>
            <p className="text-xs text-purple-200/70 font-semibold">Based on your {analytics.totalScore}/60 SNAP performance</p>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
          <div className="bg-[#3b0764]/80 p-5 rounded-2xl border border-purple-700/60 space-y-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-emerald-400">SIBM Pune</span>
            <p className="text-sm font-bold text-white leading-snug">MBA Flagship, MBA Innovation & Entr.</p>
            <p className="text-xs font-semibold text-purple-200/70 pt-1">
              Cutoff: 42+ Marks (98.5+%ile) • {analytics.totalScore >= 42 ? '✅ High Call Probability' : '⚠️ Target 42+ for SIBM Pune'}
            </p>
          </div>

          <div className="bg-[#3b0764]/80 p-5 rounded-2xl border border-purple-700/60 space-y-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-purple-300">SCMHRD Pune</span>
            <p className="text-sm font-bold text-white leading-snug">MBA Core, MBA HR, MBA Business Analytics</p>
            <p className="text-xs font-semibold text-purple-200/70 pt-1">
              Cutoff: 40+ Marks (97+%ile) • {analytics.totalScore >= 40 ? '✅ High Call Probability' : '👍 Accessible Target'}
            </p>
          </div>

          <div className="bg-[#3b0764]/80 p-5 rounded-2xl border border-purple-700/60 space-y-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-amber-300">SIIB & SIBM Bangalore</span>
            <p className="text-sm font-bold text-white leading-snug">SIIB Pune & SIBM Bengaluru</p>
            <p className="text-xs font-semibold text-purple-200/70 pt-1">
              Cutoff: 35 - 38+ Marks (90-93%ile) • {analytics.totalScore >= 35 ? '✅ High Call Probability' : '👍 Moderate Chances'}
            </p>
          </div>

          <div className="bg-[#3b0764]/80 p-5 rounded-2xl border border-purple-700/60 space-y-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-teal-300">SIOM Nashik & SCIT Pune</span>
            <p className="text-sm font-bold text-white leading-snug">SIOM Nashik, SCIT Pune, SIDTM Pune</p>
            <p className="text-xs font-semibold text-purple-200/70 pt-1">
              Cutoff: 30 - 34+ Marks (82-87%ile) • {analytics.totalScore >= 30 ? '✅ High Call Probability' : '👍 Accessible Target'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-6 md:p-12 border-2 border-slate-200 shadow-xl space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b-2 border-slate-100">
          <div>
            <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-3">
              <BookOpen className="w-8 h-8 text-purple-600" /> Complete Question Solutions & Explanations
            </h3>
            <p className="text-sm font-semibold text-slate-500 mt-1">
              Detailed step-by-step mathematical logic and analytical solutions for all 60 questions
            </p>
          </div>

          {/* Section Filter Pills */}
          <div className="flex flex-wrap items-center gap-2">
            {(['all', 'english', 'quant', 'logic'] as const).map((sec) => (
              <button
                key={sec}
              ...
              >
                {sec === 'all' ? 'All Sections (60)' : sec.toUpperCase()}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200">
          ...
          {[
            { key: 'all', label: `All (${SNAP_MOCK_TEST_60.length})` },
          ...
          {filteredQuestions.map((q) => {
            ...
            return (
              ...
              >
                  ...
                  <div>
                    {isCorrect ? (
                      ...
                      <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-rose-100 text-rose-800 font-black text-xs uppercase tracking-wider">
                        <XCircle className="w-4 h-4 text-rose-600" /> Incorrect (-0.25 Penalty)
                      </span>
                    ) : (
                ...
                <div className="bg-purple-50/60 rounded-2xl border-2 border-purple-200/80 p-5 md:p-6 space-y-2 text-xs md:text-sm text-slate-800">
                  <div className="flex items-center gap-2 text-purple-900 font-extrabold uppercase text-xs tracking-wider">
      ...
      <div className="flex flex-col sm:flex-row gap-4 pt-4 pb-16">
        ...
        >
          ...
          <span>Retake SNAP Mock Test</span>
        </button>
        <button
          onClick={() => window.print()}
          className="flex-1 bg-purple-600 hover:bg-purple-500 text-white py-5 rounded-2xl font-black text-sm uppercase tracking-widest transition-all shadow-xl shadow-purple-600/20 flex items-center justify-center gap-2 active:scale-95"
        >
