...
export type XatUserAnswers = Record<number, number>; // qId -> optionIndex (0-4)
export type QuestionStatus = 'not_visited' | 'not_answered' | 'answered' | 'marked' | 'answered_marked';

interface XatExamInterfaceProps {
  student: GenericStudentInfo;
  onComplete: (answers: XatUserAnswers) => void;
}

export function XatExamInterface({ student, onComplete }: XatExamInterfaceProps) {
  // Current active question and section
  const [currentQuestionId, setCurrentQuestionId] = useState<number>(1);
  const [activeSection, setActiveSection] = useState<'valr' | 'dm' | 'qadi' | 'gk'>('valr');
  ...
  // Comprehensive Exam Timer: 195 minutes (11,700 seconds)
  const [timeLeft, setTimeLeft] = useState<number>(195 * 60);
  ...
  const currentQuestion = useMemo(() => {
    return XAT_MOCK_TEST_95.find(q => q.id === currentQuestionId) || XAT_MOCK_TEST_95[0];
  }, [currentQuestionId]);

  // Section questions mapping
  const sectionQuestions = useMemo(() => {
    return {
      valr: XAT_MOCK_TEST_95.filter(q => q.section === 'valr'),
      dm: XAT_MOCK_TEST_95.filter(q => q.section === 'dm'),
      qadi: XAT_MOCK_TEST_95.filter(q => q.section === 'qadi'),
      gk: XAT_MOCK_TEST_95.filter(q => q.section === 'gk')
    };
  }, []);
  ...
  const handleSelectQuestion = (qId: number) => {
    const targetQ = XAT_MOCK_TEST_95.find(q => q.id === qId);
    if (targetQ) {
  ...
  const handleSaveAndNext = () => {
    ...
    if (currentIndexInActive < activeQuestions.length - 1) {
      ...
      if (activeSection === 'valr') {
        setActiveSection('dm');
        handleSelectQuestion(sectionQuestions.dm[0].id);
      } else if (activeSection === 'dm') {
        setActiveSection('qadi');
        handleSelectQuestion(sectionQuestions.qadi[0].id);
      } else if (activeSection === 'qadi') {
        setActiveSection('gk');
        handleSelectQuestion(sectionQuestions.gk[0].id);
      } else {
  ...
  const getSectionStats = (sectionKey: 'valr' | 'dm' | 'qadi' | 'gk') => {
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] text-slate-800 flex flex-col selection:bg-amber-100 selection:text-amber-900">
      {/* 1. TOP HEADER (XAT CBT Bar) */}
      <header className="bg-[#0f291e] text-white px-4 md:px-8 py-3 border-b-4 border-[#1b4332] shadow-xl sticky top-0 z-40 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="bg-emerald-600 text-white px-2.5 py-1 rounded-lg font-black text-xs uppercase tracking-wider shadow-sm">
            XAT CBT
          </div>
          <div>
        ...
        <div className="flex items-center flex-wrap gap-2 md:gap-4">
          ...
          <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border-2 font-mono font-black text-base md:text-lg shadow-inner ${
            timeLeft < 600 
              ...
              : 'bg-[#081c14] border-emerald-800 text-emerald-400'
          }`}>
            <Clock className="w-4 h-4 text-emerald-400/80" />
            <span>{formatTimer(timeLeft)}</span>
            <span className="text-[10px] uppercase font-sans text-emerald-300/70 font-bold ml-1">Left</span>
          </div>

          {/* Question Paper View */}
          <button
            onClick={() => setIsQuestionPaperOpen(true)}
            className="flex items-center gap-1.5 px-3 py-2 bg-[#1b4332] hover:bg-[#2d6a4f] border border-emerald-700/60 rounded-xl text-xs font-bold text-emerald-100 transition-all shadow-sm active:scale-95"
          >
          ...
          <button
            ...
            className="p-2 bg-[#1b4332] hover:bg-[#2d6a4f] border border-emerald-700/60 rounded-xl text-emerald-200 transition-all shadow-sm"
          ...
          <button
            ...
            className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 px-5 py-2 rounded-xl font-black text-xs uppercase tracking-wider shadow-lg shadow-emerald-950/40 transition-all active:scale-95 flex items-center gap-1.5"
          >
      ...
      {/* 2. SECTION TABS (VALR, Decision Making, QADI, General Knowledge) */}
      <nav className="bg-white border-b border-slate-200 px-4 md:px-8 py-2.5 shadow-sm flex items-center justify-between gap-4 overflow-x-auto no-scrollbar">
        <div className="flex items-center gap-2">
          {(['valr', 'dm', 'qadi', 'gk'] as const).map((secKey) => {
            ...
            const labelMap = {
              valr: 'Section I: VALR (26 Qs)',
              dm: 'Section II: Decision Making (21 Qs)',
              qadi: 'Section III: QA & DI (28 Qs)',
              gk: 'Section IV: GK (20 Qs)'
            };
      ...
      {/* 3. MAIN CBT WORKSPACE (Split Screen Layout for RC, Caselets & DI) */}
          ...
          <div className="bg-white rounded-3xl border-2 border-slate-200 shadow-sm flex-1 flex flex-col overflow-hidden">
            ...
            <div className="px-6 py-4 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <span className="px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-emerald-100 text-emerald-900">
                  {currentQuestion.section === 'gk' ? '5-Choice MCQ (+1, No Negative)' : '5-Choice MCQ (+1 Correct, -0.25 Wrong)'}
                </span>
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-slate-500">
                  {markedForReview.has(currentQuestion.id) ? (
                    ...
                    <span>XAT Official Format</span>
                  )}
                </span>
              </div>
            </div>

            {/* Split Content Area (Left: Passage / Caselet Scenario / DI Table, Right: Statement & 5 Options) */}
            <div className="flex-1 flex flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-slate-200 min-h-[440px]">
              
              {/* Context / Scenario / Passage / Data Table (If available) */}
              ...
              {/* Question Statement and 5 Options */}
              <div className={`p-6 md:p-8 flex flex-col justify-between overflow-y-auto ${
                    ...
                    {currentQuestion.options.map((opt, optIdx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left p-3.5 md:p-4 rounded-2xl border-2 transition-all flex items-start gap-4 ${
                            isSelected
                              ? 'bg-emerald-50 border-emerald-600 shadow-md ring-2 ring-emerald-500/20 text-slate-950 font-bold'
                ...
                <div className="pt-6 border-t border-slate-100 text-[11px] text-slate-400 flex items-center justify-between">
                  ...
                  <span>Question ID: XAT95-Q{currentQuestion.id}</span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[85vh] shadow-2xl flex flex-col overflow-hidden border-4 border-slate-900">
            <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-black text-base uppercase tracking-wider">XAT Official Mock Test - Full Question Paper (95 Questions)</h3>
              </div>
              <button 
          ...
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[85vh] shadow-2xl flex flex-col overflow-hidden border-4 border-slate-900">
            <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between">
              <h3 className="font-black text-base uppercase tracking-wider flex items-center gap-2">
                ...
                XAT Official Exam Instructions
              </h3>
              <button 
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-sm text-slate-700 leading-relaxed">
              <div className="bg-emerald-50 border border-emerald-200 p-4 rounded-2xl">
                <h4 className="font-black text-emerald-950 uppercase text-xs tracking-wider mb-1">XAT Exam Pattern (95 Questions)</h4>
                <ul className="list-disc list-inside text-xs space-y-1 text-emerald-900 font-medium">
                  <li><strong>Section I: Verbal & Logical Ability (VALR)</strong> - 26 Questions</li>
                  <li><strong>Section II: Decision Making (DM)</strong> - 21 Questions</li>
                  <li><strong>Section III: Quantitative Ability & DI (QADI)</strong> - 28 Questions</li>
                  <li><strong>Section IV: General Knowledge (GK)</strong> - 20 Questions</li>
                </ul>
              </div>
              ...
              <div className="space-y-2 text-xs">
                ...
                <p>2. <strong>Negative Marking:</strong> -0.25 marks for each incorrect MCQ in VALR, DM, and QADI.</p>
                <p>3. <strong>General Knowledge:</strong> No negative marking in Section IV (GK).</p>
                <p>4. <strong>Instant Solutions & Scorecard:</strong> Comprehensive XLRI call predictions and step-by-step solutions are generated immediately upon submission.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl w-full max-w-lg p-6 md:p-8 shadow-2xl border-4 border-slate-900 space-y-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-black text-lg text-slate-900 uppercase">Ready to Submit XAT Exam?</h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center text-xs">
              {(['valr', 'dm', 'qadi', 'gk'] as const).map((secKey) => {
                ...
                const titleMap = { valr: 'VALR', dm: 'DM', qadi: 'QADI', gk: 'GK' };
                return (
