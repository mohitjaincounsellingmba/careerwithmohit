...
import { NmatUserAnswers } from './NmatExamInterface';
import { GenericStudentInfo } from '@/components/GenericMockTest/GenericRegistrationForm';

interface NmatScorecardSolutionsProps {
  student: GenericStudentInfo;
  answers: NmatUserAnswers;
  onReset: () => void;
}

export function NmatScorecardSolutions({ student, answers, onReset }: NmatScorecardSolutionsProps) {
  const [sectionFilter, setSectionFilter] = useState<'all' | 'language' | 'quant' | 'logic'>('all');
  const [statusFilter, setStatusFilter] = useState<'all' | 'correct' | 'incorrect' | 'skipped'>('all');

  // Compute detailed scores and sectional analytics (NMAT has no negative marking: +3 correct, 0 wrong)
  const analytics = useMemo(() => {
    ...
    const sections: Record<'language' | 'quant' | 'logic', {
      total: number;
      attempted: number;
      correct: number;
      incorrect: number;
      ...
      language: { total: 36, attempted: 0, correct: 0, incorrect: 0, skipped: 0, score: 0, maxScore: 108, accuracy: 0, scaledScore: 60 },
      quant: { total: 36, attempted: 0, correct: 0, incorrect: 0, skipped: 0, score: 0, maxScore: 108, accuracy: 0, scaledScore: 60 },
      logic: { total: 36, attempted: 0, correct: 0, incorrect: 0, skipped: 0, score: 0, maxScore: 108, accuracy: 0, scaledScore: 60 }
    };
    ...
    // Accuracy and Sectional Scaled Score Mapping (approx 60-120 range per section in NMAT scale)
    (['language', 'quant', 'logic'] as const).forEach(secKey => {
      const s = sections[secKey];
      s.accuracy = s.attempted > 0 ? Math.round((s.correct / s.attempted) * 100) : 0;
      // Linear scaling from raw score (0-108) to NMAT section scaled score (40-120)
      s.scaledScore = Math.min(120, Math.round(40 + (s.score / 108) * 80));
    });

    const totalAttempted = correctCount + incorrectCount;
    const overallAccuracy = totalAttempted > 0 ? Math.round((correctCount / totalAttempted) * 100) : 0;
    
    // Overall NMAT Scaled Score (Sum of 3 sectional scaled scores, range ~120 - 360)
    const nmatScaledScore = sections.language.scaledScore + sections.quant.scaledScore + sections.logic.scaledScore;

    // Percentile estimation
    let overallPercentile = '80.0';
    if (nmatScaledScore >= 245) overallPercentile = '99.5';
    else if (nmatScaledScore >= 232) overallPercentile = '98.5';
    else if (nmatScaledScore >= 220) overallPercentile = '95.0';
    else if (nmatScaledScore >= 205) overallPercentile = '90.0';
    else if (nmatScaledScore >= 190) overallPercentile = '82.0';
    else if (nmatScaledScore >= 170) overallPercentile = '70.0';
    else overallPercentile = '55.0';

    return {
      totalScore,
      maxPossibleScore: 324, // 108 * 3
      nmatScaledScore,
      correctCount,
      incorrectCount,
      skippedCount,
      ...
      try {
        await fetch('/api/leads', {
          ...
          body: JSON.stringify({
            ...
            source: 'NMAT 108-Question CBT Mock Test',
            score: analytics.nmatScaledScore,
            rawScore: analytics.totalScore,
  ...
  const filteredQuestions = useMemo(() => {
    return NMAT_MOCK_TEST_108.filter(q => {
      // Section filter
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700 max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="relative z-10 flex flex-col items-center text-center space-y-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full max-w-3xl">
            <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/60 flex flex-col items-center">
              ...
              <p className="text-2xl md:text-3xl font-black text-rose-400">{analytics.overallPercentile}%</p>
            </div>
            <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/60 flex flex-col items-center">
            ...
            <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/60 flex flex-col items-center">
              ...
              <p className="text-2xl md:text-3xl font-black text-white">{analytics.totalAttempted} <span className="text-sm font-normal text-slate-400">/ 108</span></p>
            </div>
            <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/60 flex flex-col items-center">
          ...
          <div className="pt-2 flex flex-wrap justify-center gap-4">
            ...
            <button
              onClick={() => {
                const url = `https://careerwithmohit.online/tools/nmat-mock-test`;
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-2">
            <Target className="w-6 h-6 text-rose-500" /> Section-by-Section Score Analysis
          </h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {(['language', 'quant', 'logic'] as const).map((secKey) => {
            ...
            const meta = {
              language: { title: 'Section I: Language Skills', sub: 'Reading, Vocabulary & Grammar', totalQs: 36, maxMarks: 108, time: '28m' },
              quant: { title: 'Section II: Quantitative Skills', sub: 'Arithmetic, DI & Algebra', totalQs: 36, maxMarks: 108, time: '52m' },
              logic: { title: 'Section III: Logical Reasoning', sub: 'Critical & Analytical Reasoning', totalQs: 36, maxMarks: 108, time: '40m' }
            }[secKey];
              ...
              >
                <div className="space-y-4">
                  <div className="flex items-start justify-between">
                    ...
                    <span className="text-xs font-mono font-black text-rose-600 bg-rose-50 px-2.5 py-1 rounded-lg border border-rose-200">
                      {data.scaledScore}/120
                    </span>
                  </div>
      ...
      {/* 3. NMIMS & TOP B-SCHOOL CALL PREDICTOR */}
      <div className="bg-slate-900 text-white rounded-3xl p-8 border-4 border-slate-800 shadow-xl space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-xl font-black uppercase tracking-tight text-white">NMIMS & NMAT B-School Cutoff Predictor</h4>
            <p className="text-xs text-slate-400 font-semibold">Based on your {analytics.nmatScaledScore} projected NMAT scaled score</p>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
          <div className="bg-slate-800/80 p-5 rounded-2xl border border-slate-700/60 space-y-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-rose-400">NMIMS Mumbai (Core MBA)</span>
            ...
            <p className="text-xs font-semibold text-slate-400 pt-1">
              Cutoff: 232+ ({analytics.nmatScaledScore >= 232 ? '✅ High Call Probability' : '⚠️ Target 235+ for Mumbai'})
            </p>
          </div>

          <div className="bg-slate-800/80 p-5 rounded-2xl border border-slate-700/60 space-y-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-blue-400">NMIMS Bangalore & Hyderabad</span>
            <p className="text-sm font-bold text-white leading-snug">NMIMS Bengaluru, Hyderabad, Navi Mumbai, Indore</p>
            <p className="text-xs font-semibold text-slate-400 pt-1">
              Cutoff: 215 - 222+ ({analytics.nmatScaledScore >= 215 ? '✅ High Call Probability' : '👍 Accessible Target'})
            </p>
          </div>

          <div className="bg-slate-800/80 p-5 rounded-2xl border border-slate-700/60 space-y-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-emerald-400">XIMB & K J Somaiya</span>
            <p className="text-sm font-bold text-white leading-snug">XIMB (HRM), K J Somaiya Mumbai, TAPMI Manipal</p>
            <p className="text-xs font-semibold text-slate-400 pt-1">
              Cutoff: 200 - 215+ ({analytics.nmatScaledScore >= 200 ? '✅ High Call Probability' : '👍 Moderate Chances'})
            </p>
          </div>

          <div className="bg-slate-800/80 p-5 rounded-2xl border border-slate-700/60 space-y-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-purple-400">SDA Bocconi & Top Private</span>
            <p className="text-sm font-bold text-white leading-snug">SDA Bocconi Asia Center, Welingkar Mumbai, ISBR</p>
            <p className="text-xs font-semibold text-slate-400 pt-1">
              Cutoff: 185 - 200+ ({analytics.nmatScaledScore >= 185 ? '✅ High Call Probability' : '👍 Accessible Target'})
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-6 md:p-12 border-2 border-slate-200 shadow-xl space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b-2 border-slate-100">
          <div>
            <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-3">
              <BookOpen className="w-8 h-8 text-rose-500" /> Complete Question Solutions & Explanations
            </h3>
            <p className="text-sm font-semibold text-slate-500 mt-1">
              Detailed step-by-step mathematical rationale and logical analysis for all 108 questions
            </p>
          </div>

          {/* Section Filter Pills */}
          <div className="flex flex-wrap items-center gap-2">
            {(['all', 'language', 'quant', 'logic'] as const).map((sec) => (
              <button
                key={sec}
              ...
              >
                {sec === 'all' ? 'All Sections (108)' : sec.toUpperCase()}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200">
          ...
          {[
            { key: 'all', label: `All (${NMAT_MOCK_TEST_108.length})` },
          ...
          {filteredQuestions.map((q) => {
            ...
            return (
              ...
              >
                  ...
                  <div>
                    {isCorrect ? (
                      ...
                      <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-rose-100 text-rose-800 font-black text-xs uppercase tracking-wider">
                        <XCircle className="w-4 h-4 text-rose-600" /> Incorrect (0 Penalty)
                      </span>
                    ) : (
                ...
                <div className="bg-rose-50/60 rounded-2xl border-2 border-rose-200/80 p-5 md:p-6 space-y-2 text-xs md:text-sm text-slate-800">
                  <div className="flex items-center gap-2 text-rose-800 font-extrabold uppercase text-xs tracking-wider">
      ...
      <div className="flex flex-col sm:flex-row gap-4 pt-4 pb-16">
        ...
        >
          ...
          <span>Retake NMAT Mock Test</span>
        </button>
        <button
          onClick={() => window.print()}
          className="flex-1 bg-rose-600 hover:bg-rose-500 text-white py-5 rounded-2xl font-black text-sm uppercase tracking-widest transition-all shadow-xl shadow-rose-600/20 flex items-center justify-center gap-2 active:scale-95"
        >
