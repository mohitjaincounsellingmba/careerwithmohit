...
export type AtmaUserAnswers = Record<number, number>; // qId -> optionIndex (0-3)
export type QuestionStatus = 'not_visited' | 'not_answered' | 'answered' | 'marked' | 'answered_marked';

export type AtmaSectionKey = 'analytical-1' | 'quant-1' | 'verbal-1' | 'analytical-2' | 'quant-2' | 'verbal-2';

interface AtmaExamInterfaceProps {
  student: GenericStudentInfo;
  onComplete: (answers: AtmaUserAnswers) => void;
}

export const ATMA_SECTIONS: { id: AtmaSectionKey; label: string; short: string; qRange: string }[] = [
  { id: 'analytical-1', label: 'Analytical Reasoning I', short: 'AR-I', qRange: 'Q1-Q30' },
  { id: 'quant-1', label: 'Quantitative Skills I', short: 'QA-I', qRange: 'Q31-Q60' },
  { id: 'verbal-1', label: 'Verbal Skills I', short: 'VA-I', qRange: 'Q61-Q90' },
  { id: 'analytical-2', label: 'Analytical Reasoning II', short: 'AR-II', qRange: 'Q91-Q120' },
  { id: 'quant-2', label: 'Quantitative Skills II', short: 'QA-II', qRange: 'Q121-Q150' },
  { id: 'verbal-2', label: 'Verbal Skills II', short: 'VA-II', qRange: 'Q151-Q180' },
];

export function AtmaExamInterface({ student, onComplete }: AtmaExamInterfaceProps) {
  const [currentQuestionId, setCurrentQuestionId] = useState<number>(1);
  const [activeSection, setActiveSection] = useState<AtmaSectionKey>('analytical-1');
  ...
  // Total 180 Minutes (10,800 seconds)
  ...
  const currentQuestion = useMemo(() => {
    return ATMA_MOCK_TEST_180.find(q => q.id === currentQuestionId) || ATMA_MOCK_TEST_180[0];
  }, [currentQuestionId]);

  // Questions grouped by section
  const sectionQuestions = useMemo(() => {
    const map: Record<AtmaSectionKey, AtmaQuestion[]> = {
      'analytical-1': [],
      'quant-1': [],
      'verbal-1': [],
      'analytical-2': [],
  ...
  const handleSelectQuestion = (qId: number) => {
    const targetQ = ATMA_MOCK_TEST_180.find(q => q.id === qId);
    if (targetQ) {
  ...
  const handleSaveAndNext = () => {
    ...
    if (currentIndexInActive < activeQuestions.length - 1) {
      ...
      const secIdx = ATMA_SECTIONS.findIndex(s => s.id === activeSection);
      if (secIdx < ATMA_SECTIONS.length - 1) {
        const nextSec = ATMA_SECTIONS[secIdx + 1];
  ...
  const getSectionStats = (sectionKey: AtmaSectionKey) => {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 flex flex-col selection:bg-emerald-100 selection:text-emerald-900">
      {/* 1. TOP HEADER (ATMA Official CBT Bar) */}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-extrabold text-sm md:text-base leading-tight tracking-tight">
              ATMA Official Pattern Mock Test <span className="text-emerald-400 font-normal hidden sm:inline">(180 Questions | 180 Mins)</span>
            </h1>
        ...
        <div className="flex items-center flex-wrap gap-2 md:gap-4">
          <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border-2 font-mono font-black text-base md:text-lg shadow-inner ${
            timeLeft < 900 
              ...
              : 'bg-[#1e293b] border-slate-700 text-emerald-300'
          }`}>
      ...
      {/* 2. SECTION TABS (6 Timed Sections) */}
            ...
            <div className="px-6 py-4 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className="bg-slate-900 text-white px-3 py-1 rounded-lg font-black text-xs uppercase tracking-widest">
                  Question {currentQuestion.questionNumber} of 180
                </span>
            ...
            <div className="flex-1 flex flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-slate-200 min-h-[440px]">
              ...
              <div className={`p-6 md:p-8 flex flex-col justify-between overflow-y-auto ${
                ...
                <div className="pt-6 border-t border-slate-100 text-[11px] text-slate-400 flex items-center justify-between">
                  <span>Standard ATMA Marking (+1 / −0.25)</span>
                  <span>Question ID: ATMA180-Q{currentQuestion.id}</span>
                </div>
              </div>
            </div>
        ...
        <aside className={`lg:w-80 bg-white border-l-2 border-slate-200 p-6 flex flex-col justify-between overflow-y-auto ${
                ...
                return (
                  <button
                    ...
                    className={`h-9 rounded-xl font-mono font-black text-xs border-2 transition-all flex items-center justify-center relative ${btnClass} ${
                      isCurrent ? 'ring-4 ring-emerald-400 scale-105 z-10' : ''
                    }`}
                  >
          ...
          <div className="pt-6 mt-6 border-t border-slate-200 space-y-3">
            ...
            <p className="text-[10px] text-center text-slate-400">
              ATMA Official 6-Section Timed Simulation.
            </p>
          </div>
        </aside>
      </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[85vh] shadow-2xl flex flex-col overflow-hidden border-4 border-slate-900">
            <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-black text-base uppercase tracking-wider">ATMA 2026 - Full Question Paper (180 Questions)</h3>
              </div>
              <button 
          ...
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[85vh] shadow-2xl flex flex-col overflow-hidden border-4 border-slate-900">
            <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between">
              <h3 className="font-black text-base uppercase tracking-wider flex items-center gap-2">
                ...
                ATMA 2026 Official Instructions
              </h3>
              <button 
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-sm text-slate-700 leading-relaxed">
              <div className="bg-emerald-50 border border-emerald-200 p-4 rounded-2xl">
                <h4 className="font-black text-emerald-950 uppercase text-xs tracking-wider mb-1">ATMA Exam Pattern (180 Questions | 180 Mins)</h4>
                <ul className="list-disc list-inside text-xs space-y-1 text-emerald-900 font-medium">
                  <li><strong>Analytical Reasoning (Part I & II):</strong> 60 Questions (60 Mins)</li>
                  <li><strong>Quantitative Skills (Part I & II):</strong> 60 Questions (60 Mins)</li>
                  <li><strong>Verbal Skills (Part I & II):</strong> 60 Questions (60 Mins)</li>
                </ul>
              </div>
              ...
              <div className="space-y-2 text-xs">
                ...
                <p>4. <strong>Cutoff Matcher:</strong> Real-time JBIMS, SIMSREE, PUMBA, and Welingkar percentile predictions delivered upon submission.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl w-full max-w-lg p-6 md:p-8 shadow-2xl border-4 border-slate-900 space-y-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-black text-lg text-slate-900 uppercase">Submit ATMA Exam?</h3>
