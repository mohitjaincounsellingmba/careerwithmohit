...
import { ATMA_MOCK_TEST_180, AtmaQuestion } from '@/data/atma_mock_test_180';
import { AtmaUserAnswers, AtmaSectionKey, ATMA_SECTIONS } from './AtmaExamInterface';
import { GenericStudentInfo } from '@/components/GenericMockTest/GenericRegistrationForm';

interface AtmaScorecardSolutionsProps {
  student: GenericStudentInfo;
  answers: AtmaUserAnswers;
  onReset: () => void;
}

export function AtmaScorecardSolutions({ student, answers, onReset }: AtmaScorecardSolutionsProps) {
  const [sectionFilter, setSectionFilter] = useState<'all' | AtmaSectionKey>('all');
  ...
  const analytics = useMemo(() => {
    ...
    const sections: Record<AtmaSectionKey, {
      total: number;
      attempted: number;
      correct: number;
      incorrect: number;
      ...
      'analytical-1': { total: 30, attempted: 0, correct: 0, incorrect: 0, skipped: 0, score: 0, accuracy: 0 },
      'quant-1': { total: 30, attempted: 0, correct: 0, incorrect: 0, skipped: 0, score: 0, accuracy: 0 },
      'verbal-1': { total: 30, attempted: 0, correct: 0, incorrect: 0, skipped: 0, score: 0, accuracy: 0 },
      'analytical-2': { total: 30, attempted: 0, correct: 0, incorrect: 0, skipped: 0, score: 0, accuracy: 0 },
      'quant-2': { total: 30, attempted: 0, correct: 0, incorrect: 0, skipped: 0, score: 0, accuracy: 0 },
      'verbal-2': { total: 30, attempted: 0, correct: 0, incorrect: 0, skipped: 0, score: 0, accuracy: 0 }
    };
    ...
    // Percentile estimation benchmarked against historical AIMS ATMA data
    let percentile = '70.0';
    if (totalScore >= 140) percentile = '99.95';
    else if (totalScore >= 130) percentile = '99.50';
    else if (totalScore >= 120) percentile = '98.20';
    else if (totalScore >= 110) percentile = '96.00';
    else if (totalScore >= 100) percentile = '92.50';
    else if (totalScore >= 90) percentile = '87.00';
    else if (totalScore >= 80) percentile = '80.00';
    else if (totalScore >= 70) percentile = '72.00';
    else if (totalScore >= 60) percentile = '62.00';
    else percentile = '45.00';
  ...
  const filteredQuestions = useMemo(() => {
    return ATMA_MOCK_TEST_180.filter(q => {
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700 max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="bg-[#0f172a] text-white rounded-[3rem] p-8 md:p-14 shadow-2xl border-4 border-slate-800 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center text-center space-y-8">
          <div className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 px-6 py-2 rounded-full font-black text-xs uppercase tracking-[0.25em]">
            <Award className="w-4 h-4" /> Official ATMA Scorecard & B-School Matcher
          </div>
          ...
          <div>
            ...
            <div className="flex items-baseline justify-center gap-2">
              <span className={`text-6xl md:text-8xl font-black tracking-tight ${
                analytics.totalScore >= 120 ? 'text-emerald-400' : analytics.totalScore >= 95 ? 'text-indigo-400' : 'text-amber-400'
              }`}>
                {analytics.totalScore}
              </span>
              <span className="text-slate-600 text-3xl md:text-5xl font-black">/ 180.00</span>
            </div>
            <p className="text-xs text-slate-400 font-semibold mt-2">
              Marking: <strong className="text-white">+1.00 for Correct</strong> • <strong className="text-rose-400">−0.25 for Incorrect</strong>
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full max-w-3xl">
            <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/60 flex flex-col items-center">
              ...
              <p className="text-2xl md:text-3xl font-black text-emerald-400">{analytics.percentile}%</p>
            </div>
            <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/60 flex flex-col items-center">
              <p className="text-[10px] font-black uppercase text-slate-400 tracking-wider mb-1">Accuracy Rate</p>
              <p className="text-2xl md:text-3xl font-black text-indigo-400">{analytics.overallAccuracy}%</p>
            </div>
            <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/60 flex flex-col items-center">
              <p className="text-[10px] font-black uppercase text-slate-400 tracking-wider mb-1">Attempted</p>
              <p className="text-2xl md:text-3xl font-black text-white">{analytics.totalAttempted} <span className="text-sm font-normal text-slate-400">/ 180</span></p>
            </div>
            <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700/60 flex flex-col items-center">
          ...
          <p className="text-slate-300 font-medium text-base md:text-lg max-w-2xl leading-relaxed">
            Candidate <strong className="text-white">{student.name}</strong>, your raw score of <strong className="text-emerald-400">{analytics.totalScore}</strong> translates to an estimated <strong className="text-emerald-400">{analytics.percentile} percentile</strong> in ATMA 2026.
          </p>
          ...
          <div className="pt-2 flex flex-wrap justify-center gap-4">
            <button
              onClick={() => {
                const text = `I just scored ${analytics.totalScore}/180 (${analytics.percentile}%ile) in the ATMA 2026 Full CBT Mock Test on CareerWithMohit! 🎯 Check your JBIMS & PUMBA cutoff readiness: https://careerwithmohit.online/tools/mock-test/atma/`;
            ...
            <button
              onClick={() => {
                const url = `https://careerwithmohit.online/tools/mock-test/atma/`;
      ...
      {/* 2. 6-SECTION SCORECARDS */}
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-2">
            <Target className="w-6 h-6 text-emerald-600" /> 6 Timed Sections Breakdown (30 Marks Each)
          </h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {ATMA_SECTIONS.map((sec) => {
      ...
      <div className="bg-[#0f172a] text-white rounded-3xl p-8 border-4 border-slate-800 shadow-xl space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-xl font-black uppercase tracking-tight text-white">Top MBA Institutes Accepting ATMA</h4>
            <p className="text-xs text-slate-400 font-semibold">Based on your {analytics.totalScore}/180 score ({analytics.percentile}%ile)</p>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
          <div className="bg-slate-800/80 p-5 rounded-2xl border border-slate-700/60 space-y-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-emerald-400">JBIMS Mumbai</span>
            <p className="text-sm font-bold text-white leading-snug">Jamnalal Bajaj Institute of Management Studies</p>
            <p className="text-xs font-semibold text-slate-400 pt-1">
              Target: 140+ Marks (99.9%ile) • {analytics.totalScore >= 140 ? '✅ High Admission Probability' : '⚠️ Requires 140+ Score'}
            </p>
          </div>

          <div className="bg-slate-800/80 p-5 rounded-2xl border border-slate-700/60 space-y-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-indigo-400">SIMSREE Mumbai</span>
            <p className="text-sm font-bold text-white leading-snug">Sydenham Institute of Management Studies</p>
            <p className="text-xs font-semibold text-slate-400 pt-1">
              Target: 130+ Marks (99.5%ile) • {analytics.totalScore >= 130 ? '✅ Strong Eligibility' : '👍 Accessible Target'}
            </p>
          </div>

          <div className="bg-slate-800/80 p-5 rounded-2xl border border-slate-700/60 space-y-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-amber-400">PUMBA Pune</span>
            <p className="text-sm font-bold text-white leading-snug">Dept of Management Sciences, Pune University</p>
            <p className="text-xs font-semibold text-slate-400 pt-1">
              Target: 120+ Marks (98.0%ile) • {analytics.totalScore >= 120 ? '✅ High Call Chance' : '👍 Competitive Target'}
            </p>
          </div>

          <div className="bg-slate-800/80 p-5 rounded-2xl border border-slate-700/60 space-y-2">
            <span className="text-[10px] font-black uppercase tracking-widest text-purple-400">Welingkar (WeSchool)</span>
            <p className="text-sm font-bold text-white leading-snug">Welingkar Mumbai & Bengaluru</p>
            <p className="text-xs font-semibold text-slate-400 pt-1">
              Target: 110+ Marks (96.0%ile) • {analytics.totalScore >= 110 ? '✅ Direct Call Expected' : '🚀 Target 110+'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-6 md:p-12 border-2 border-slate-200 shadow-xl space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b-2 border-slate-100">
          <div>
            <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-3">
              <BookOpen className="w-8 h-8 text-emerald-600" /> Complete Question Solutions & Detailed Explanations
            </h3>
            <p className="text-sm font-semibold text-slate-500 mt-1">
              Step-by-step mathematical proofs, logical deduction steps, and verbal rationales for all 180 questions
            </p>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200">
          ...
          {[
            { key: 'all', label: `All (${ATMA_MOCK_TEST_180.length})` },
      ...
      <div className="flex flex-col sm:flex-row gap-4 pt-4 pb-16">
        ...
        >
          ...
          <span>Retake ATMA 2026 Mock Test</span>
        </button>
        <button
          onClick={() => window.print()}
