...
const SECTIONS = [
  {
    key: "valr",
    label: "VALR",
    fullName: "Verbal and Logical Ability",
    totalMcq: 26,
  ...
  },
  {
    key: "qa",
    label: "QA",
    fullName: "Quantitative Ability & Data Interpretation",
    totalMcq: 28,
...
// Percentile lookup table based on XAT historical trends
function estimatePercentile(totalRaw: number): number {
  if (totalRaw >= 45) return 99.9;
  if (totalRaw >= 40) return 99.5;
  if (totalRaw >= 38) return 99.0;
  if (totalRaw >= 35) return 98.0;
  if (totalRaw >= 30) return 95.0;
  if (totalRaw >= 27) return 90.0;
  if (totalRaw >= 24) return 85.0;
  if (totalRaw >= 22) return 80.0;
  if (totalRaw >= 19) return 75.0;
  if (totalRaw >= 16) return 70.0;
  if (totalRaw >= 14) return 60.0;
  if (totalRaw >= 10) return 50.0;
  return 30.0;
}

function estimateSectionPercentile(rawScore: number, section: string): number {
  const table: Record<string, Array<[number, number]>> = {
    valr: [[16,99],[14,95],[11,90],[9,85],[7,80],[6,70]],
    dm:   [[15,99],[12,95],[10,90],[8,85],[7,80],[5,70]],
    qa:   [[17,99],[14,95],[11,90],[9,85],[7,80],[5,70]],
  };
...
function calcSectionRaw(input: SectionInput) {
  ...
  return cMcq * 1 - wMcq * 0.25;
}

export function XatScoreCalculator() {
  ...
  const [activeTab, setActiveTab] = useState<SectionKey>("valr");
  ...
  const stats = useMemo(() => {
    ...
    const sections = SECTIONS.map((s) => {
      const cMcq = Number(inputs[s.key as SectionKey].correctMcq) || 0;
      const wMcq = Number(inputs[s.key as SectionKey].wrongMcq) || 0;
      const attempted = cMcq + wMcq;
      unattemptedCount += (s.totalMcq - attempted);

      const raw = calcSectionRaw(inputs[s.key as SectionKey]);
      const maxRaw = s.totalMcq * 1;
      const percentile = estimateSectionPercentile(raw, s.key);
      return { key: s.key, raw, maxRaw, percentile };
    });

    let totalRaw = sections.reduce((a, b) => a + b.raw, 0);
    const maxTotal = sections.reduce((a, b) => a + b.maxRaw, 0);
    
    // Unattempted deduction: -0.10 for every unattempted question beyond 8
    const unattemptedPenalty = Math.max(0, unattemptedCount - 8) * 0.10;
    totalRaw = totalRaw - unattemptedPenalty;

    const overallPercentile = estimatePercentile(totalRaw);
    
    return { sections, totalRaw: parseFloat(totalRaw.toFixed(2)), maxTotal, overallPercentile, unattemptedPenalty: parseFloat(unattemptedPenalty.toFixed(2)) };
  }, [inputs]);
  ...
  const reset = () => {
    setInputs({ valr: { ...defaultSection }, dm: { ...defaultSection }, qa: { ...defaultSection } });
    setIsUnlocked(false);
  ...
  const hasAnyInput = SECTIONS.some((s) => {
    ...
    return inp.correctMcq !== "" || inp.wrongMcq !== "";
  });
    ...
    <div className="w-full max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-amber-300 font-bold text-sm uppercase tracking-widest mt-1">
                Section-wise · Penalty · Percentile
              </p>
            </div>
          </div>
          ...
          <div className={`${currentSection.bg} border-4 ${currentSection.border} p-6 md:p-8 mb-8`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                />
                <p className="text-[10px] text-emerald-700 font-black uppercase mt-1">+1 each</p>
              </div>
              ...
              <div>
                ...
                />
                <p className="text-[10px] text-rose-600 font-black uppercase mt-1">−0.25 each</p>
              </div>
            </div>
          ...
          {showLeadForm && !isUnlocked && (
            <div className="border-4 border-foreground bg-foreground text-white p-8 mb-8 animate-in slide-in-from-bottom-4 duration-300">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xl font-black uppercase">Unlock Your XAT Score & Percentile</h3>
              </div>
              <form onSubmit={handleLeadSubmit} className="space-y-4">
                ...
                >
                  Reveal My XAT Score <ChevronRight className="w-5 h-5" />
                </button>
                <button
                  type="button"
              ...
              <div className="bg-foreground text-white p-8 border-b-[12px] border-amber-400 relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <span className="text-xs font-black uppercase tracking-[0.2em] text-amber-300 block mb-3 animate-pulse">
                    Overall Raw Score (After penalty)
                  </span>
                  ...
                  <div className="mt-4 flex flex-wrap gap-4">
                    ...
                    {stats.unattemptedPenalty > 0 && (
                      <div className="bg-white/10 border border-white/20 px-4 py-2">
                        <div className="text-[10px] font-black uppercase text-amber-300">Unattempted Penalty</div>
                        <div className="text-2xl font-black text-rose-400">-{stats.unattemptedPenalty}</div>
                      </div>
                    )}
                  </div>
                </div>
              ...
              >
                <div>
                  ...
                  <div className="text-xl font-black uppercase">
                    Book Free XAT 2027 Counselling →
                  </div>
                </div>
          ...
          <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-[32px_32px_0px_0px_rgba(0,0,0,1)] border-8 border-foreground text-foreground">
            ...
            <div className="bg-white">
              <div className="bg-amber-400 p-8 text-center border-b-8 border-foreground text-foreground">
                <h2 className="text-4xl font-black uppercase italic tracking-tighter">XAT Admission Support 2027</h2>
                <p className="font-bold mt-2">Expert guidance for XLRI & Top B-Schools.</p>
              </div>
      ...
      <div className="mt-20">
        <h3 className="text-3xl font-black uppercase tracking-tight mb-8 border-l-[12px] border-amber-400 pl-6">
          XAT 2027 Score vs Percentile (Expected)
        </h3>
        <div className="overflow-x-auto border-4 border-foreground">
          <table className="w-full text-left border-collapse">
            <thead className="bg-foreground text-white uppercase text-sm font-black tracking-widest">
              <tr>
                <th className="p-5 border-r border-white/20">Raw Score (out of 75)</th>
            ...
            <tbody className="text-base font-bold">
              {[
                ["40 – 75", "99.5+", "XLRI Jamshedpur"],
                ["35 – 39", "98.0 – 99.4", "XIMB, IMT, IMI"],
                ["30 – 34", "95.0 – 97.9", "GIM, TAPMI, GLIM"],
                ["27 – 29", "90.0 – 94.9", "KJ Somaiya, IRMA, MICA"],
                ["24 – 26", "85.0 – 89.9", "BIMTECH, FORE, LIBA"],
                ["19 – 23", "75.0 – 84.9", "Tier 2 B-schools"],
                ["10 – 18", "50.0 – 74.9", "Tier 3 B-schools"],
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-base font-bold">
              ...
              <tr className="bg-foreground text-white">
                ...
                <td className="p-5 border-r-4 border-white/20">75</td>
                <td className="p-5 border-r-4 border-white/20">75</td>
              </tr>
            </tbody>
          </table>
