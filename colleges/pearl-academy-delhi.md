---
name: "Pearl Academy, Delhi"
logo: "/colleges/pearl-logo.webp"
location: "South Delhi, Delhi"
established: 1993
ownership: "Private"
ranking: "#1 Fashion & Design Institute in India"
fees: "₹10.93 - 15.27 Lakhs"
avg_placement: "₹8.0 - 10.0 LPA"
highest_placement: "₹26.00 LPA"
exams: ["Pearl Academy Entrance", "CAT", "MAT"]
website: "https://pearlacademy.com"
---

### Courses & Fees
- **MBA**: Fashion Business | 2 Years | ₹12.20 Lakhs
- **MBA**: Global Business Mgmt | 2 Years | ₹11.55 Lakhs
- **MBA**: Luxury Brand Mgmt | 2 Years | ₹11.85 Lakhs
- **MBA**: Event Management | 2 Years | ₹12.20 Lakhs

### About Pearl Academy Delhi
Pearl Academy is a global leader in creative education, offering specialized MBA programs in Design, Fashion, and Luxury Management. The Delhi campus is a hub for creativity and innovation, providing students with world-class infrastructure and industry exposure. The MBA programs are designed for those looking to lead in creative industries.

### Creative MBA Programs
- **MBA (Fashion & Lifestyle)**: Focus on the business of fashion.
- **MBA (Design Strategy)**: Integrating design thinking into management.
- **MBA (Luxury Brand Management)**: Specialized track for the high-end luxury sector.

### Placements 2024-25
- **Highest Package**: ₹26.00 LPA
- **Placement Rate**: 99%
- **Recruiters**: Over 700 companies including top fashion houses, design firms, and MNCs like Amazon and Burberry.

### Admission Process
1. **Pearl Entrance**: A creative and aptitude-based entrance test.
2. **Portfolio Review**: Critical for creative management tracks.
3. **Personal Interview**: Final selection based on vision and creative potential.
