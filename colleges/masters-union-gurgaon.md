---
name: "Masters' Union, Gurgaon"
logo: "/colleges/masters-union-logo.webp"
location: "Gurgaon, Haryana"
established: 2020
ownership: "Private"
ranking: "Top Innovative B-School in India"
fees: "₹29.38 - 29.73 Lakhs"
avg_placement: "₹34.07 LPA"
highest_placement: "₹61.80 LPA"
exams: ["CAT", "GMAT", "GRE", "MU-BA"]
website: "https://mastersunion.org"
---

### About Masters' Union
Masters' Union is a premium, technology-first business school located in Cyber City, Gurgaon. It is unique for its "Practitioner-led" model where courses are taught by CEOs, MDs, and industry veterans rather than traditional academics. The curriculum focuses heavily on hands-on experience, startup incubation, and real-world projects.

### Courses & Fees
- **PGP TBM**: Tech Business Management | 16 Months | ₹25.00 Lakhs
- **Executive PGP**: For Working Professionals | 12 Months | ₹18.00 Lakhs
- **UG Programme**: Business & Management | 4 Years | ₹32.00 Lakhs
| Masters Camp | Product Management | 6 Months | ₹2.50 Lakhs |

### PGP TBM (Postgraduate Program in Technology and Business Management)
This is the flagship program equivalent to an MBA, designed for students who want to lead in the tech-first world.
- **Practitioner-led Learning**: Learn from leaders at Google, Amazon, and top startups.
- **Venture Lab**: Build and scale your own startup as part of the curriculum.
- **Investment Fund**: Manage a real investment fund during the program.

### Placements 2024-25
Masters' Union has achieved industry-leading placement statistics.
- **Highest Package**: ₹61.80 LPA
- **Average Package**: ₹34.07 LPA
- **Top 25% Average**: ₹44.67 LPA
- **Median Salary Increase**: 3.03x (Pre-MBA vs Post-MBA)
- **Top Recruiters**: Bain & Co, McKinsey, Google, Microsoft, Softbank, and top-tier Startups.

### Admission Process
1. **MU-BA Entrance**: A proprietary entrance test covering business aptitude.
2. **Profile Evaluation**: Strong emphasis on achievements and leadership potential.
3. **Panel Interview**: Interaction with industry leaders.
