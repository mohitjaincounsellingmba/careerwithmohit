---
name: "Fostiima Business School, Delhi"
logo: "/colleges/fostiima-logo.webp"
location: "Dwarka, Delhi"
established: 2007
ownership: "Private"
ranking: "#1 B-School in Delhi/NCR for ROI"
fees: "₹10.5 - 12.0 Lakhs"
avg_placement: "₹9.0 - 10.0 LPA"
highest_placement: "₹25.00 LPA"
exams: ["CAT", "MAT", "XAT", "CMAT"]
website: "https://fostiima.org"
---

### About Fostiima Delhi
Fostiima Business School was founded by alumni of IIM Ahmedabad (IIMA) to provide IIM-standard education. The school is located in Dwarka, Delhi, and is known for its "Elite" batch programs which claim high placement packages. It focuses on a practical, case-study based curriculum led by an expert faculty.

### PGDM Elite Batch
- **Flagship Program**: Designed to match IIM standards.
- **IIM Alumni Faculty**: Learn from professors who have studied at top global b-schools.
- **ROI Focus**: One of the best fee-to-placement ratios in Delhi.

### Placements 2024-25
- **Highest Package**: ₹25.00 LPA
- **Average Package**: ₹10.0 LPA
- **Median Package**: ₹11.5 LPA
- **Placement Rate**: 100%
- **Top Recruiters**: Deloitte, KPMG, Axis Bank, ICICI, BYJU'S, HDFC.

### Admission Process
1. **Entrance Score**: Merit in CAT/MAT/XAT/CMAT.
2. **GD/PI**: Multi-stage selection process including group discussions.
3. **Scholarships**: Available for top scorers in entrance exams.
