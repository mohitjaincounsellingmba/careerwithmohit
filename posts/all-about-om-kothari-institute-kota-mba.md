---
title: "All About Om Kothari Institute (OKIMR) Kota MBA: Placements & Reviews 2026"
date: "2026-03-18"
description: "Detailed review of Om Kothari Institute of Management and Research (OKIMR) Kota MBA. Explore the ₹1.2 Lakhs fees and 100% placement support."
keywords: ["Om Kothari Institute Kota MBA review", "OKIMR Kota fees 2026", "OKIMR placement records", "best B-school in Kota", "Om Kothari MBA admission"]
---

### 📊 Om Kothari (OKIMR) Kota 2026 Snapshot

| Category | Details |
| :--- | :--- |
| **Total Fees (2-Year)** | ~₹1.20 Lakhs |
| **Highest Package** | ₹3.84 LPA |
| **Average Placement** | ₹3.50 LPA |
| **Entrance Accepted** | MAT, CMAT, RMAP |
| **Top Recruiter** | Axis Bank, ICICI, Om Toyota |

---

## 🏛️ Why OKIMR is the Regional Workhorse in Kota?
Om Kothari Institute (OKIMR) has been a stable pillar of management education in Kota for decades, focusing on creating career-ready graduates for the regional job market.
*   **100% Placement Support:** A dedicated cell ensures every student gets multiple interview opportunities in local and regional sectors.
*   **Strong Alumni Base:** Over 2,500+ alumni working in top positions across India and abroad.
*   **Affordable Quality:** One of the few private institutes whose fees match that of government colleges.

---

## ❓ Frequently Asked Questions (FAQ)

### 1. How is the placement at Om Kothari Institute Kota?
Placements are highly focused on the **Banking and Service sector**. Axis Bank, ICICI, and Om Toyota are regular recruiters with a stable average package around ₹3.5 LPA.

### 2. Is OKIMR affiliated with RTU?
Yes, OKIMR is usually affiliated with Rajasthan Technical University (RTU), Kota, providing a degree that is recognized across India and by government bodies.

### 3. What is the total fee for MBA at Om Kothari in 2026?
The tuition fee is approximately **₹60,000 per year**, totaling around **₹1.2 Lakhs** for the entire program.

---

**Our Verdict**: If you want a long-standing academic brand with stable placements and low private fees, **OKIMR is a very solid local choice.**

[👉 Apply to OKIMR Kota](/inquiry) | [👉 Get Free Guidance](/inquiry)
