---
title: >-
  How to Check MAT May 2026 PBT Score – Scorecard Download, Result Date &
  Calculator
date: '2026-06-02'
description: >-
  Comprehensive expert analysis and 2026-2027 admission guide for How to Check
  MAT May 2026 PBT Score – Scorecard Download, Result Date & Calculator. Check
  updated fees, placement records, real cutoffs, and selection tips by Mohit
  Jain.
keywords:
  - MAT May 2026 PBT score
  - check MAT PBT result 2026
  - MAT PBT scorecard download
  - mat.aima.in PBT result
  - MAT May 2026 paper based test result
  - AIMA MAT PBT score 2026
  - MAT score calculator
  - MAT PBT result date June 2026
  - Best Colleges in Noida
  - Noida Admissions 2026
  - Direct Admission in Noida
  - Noida Colleges
  - Delhi NCR Colleges
  - Best Colleges in Delhi NCR
  - Direct Admission Delhi NCR
  - Delhi NCR College Counselling
faqs:
  - question: Where can I check my MAT PBT scorecard?
    answer: >-
      Only at the official AIMA portal: **[mat.aima.in](https://mat.aima.in)** →
      Candidate Login → Dashboard → Scorecard.
  - question: Is the scorecard sent by email?
    answer: >-
      No. AIMA does not email or post scorecards. You must log in and download
      it yourself.
  - question: Can I use my MAT PBT score for CBT-mode colleges?
    answer: >-
      Yes. MAT score is mode-agnostic — colleges accept scores regardless of
      whether you took PBT, CBT, or IBT.
  - question: How long is the MAT PBT score valid?
    answer: Your score is valid for **12 months** from the date of declaration.
  - question: What is the maximum composite score in MAT?
    answer: >-
      The maximum composite score is **800** (200 per section × 4 sections: LC,
      ICR, MS, DA).
location: Noida
state: Delhi NCR
---

The **MAT May 2026 Paper-Based Test (PBT)** was held on **May 31, 2026** across multiple test centres in India. If you appeared for the PBT mode, you are now eagerly waiting for your official scorecard. This guide covers everything — the result date, how to check and download your PBT scorecard, and how to make sense of your score.

---

## 🧮 MAT Score Calculator — Know Your Score Instantly

**Before your official result arrives, calculate your estimated MAT composite score right now!**

<div class="calculator-cta-top">

> 🎯 **[Use Free MAT May 2026 Score Calculator →](/tools/mat-score-calculator)**
>
> Enter your correct & incorrect answers across all 4 sections (LC, MS, DA, ICR) and instantly get your estimated **composite score out of 800** and **expected percentile**. No login required!

</div>

---

## MAT May 2026 PBT Result: Expected Date

The **MAT PBT (May 31, 2026)** result is expected approximately **10–15 days** after the exam. Based on AIMA's consistent historical pattern:

| Event | Expected Timeline |
| :--- | :--- |
| **MAT PBT Exam Date** | May 31, 2026 |
| **Result Declaration (PBT)** | June 14–20, 2026 |
| **Scorecard Availability** | Same day as result |
| **Score Validity** | 12 months from result date |

> **Important:** AIMA does **not** pre-announce the exact result date. Monitor [mat.aima.in](https://mat.aima.in) daily from **June 14, 2026** onwards. Results appear directly in your candidate dashboard — no SMS or email alert is sent.

---

## What is the MAT PBT Mode?

The **Paper-Based Test (PBT)** is the traditional pen-and-paper format of the Management Aptitude Test conducted by the **All India Management Association (AIMA)**. Unlike the CBT (Computer-Based Test) or IBT (Internet-Based Test), the PBT is conducted at **designated test centres** across India.

Key facts about the PBT mode:
- **Duration:** 2.5 hours (150 minutes)
- **Total Questions:** 150 MCQs
- **Sections:** Language Comprehension (30), Intelligence & Critical Reasoning (30), Mathematical Skills (30), Data Analysis & Sufficiency (30), Indian & Global Environment (30)
- **Marking:** +1 for correct, −0.25 for wrong
- **Composite Score:** Out of 800 (IGE section excluded from composite)

---

## How to Check MAT May 2026 PBT Score: Step-by-Step

Once AIMA officially releases the PBT result, follow these steps to access your scorecard:

### Step 1: Go to the Official AIMA MAT Portal
Visit **[mat.aima.in](https://mat.aima.in)** — this is the **only authentic source** for your MAT scorecard. Avoid any third-party websites claiming to show results early.

### Step 2: Click on "Candidate Login"
On the homepage, locate the **Candidate Login** section. Do not use social login or third-party shortcuts.

### Step 3: Enter Your Credentials
Log in using:
- **Registered Email ID** (the one used during MAT registration)
- **Password**

Forgot your password? Click **"Forgot Password"** and reset it using your registered email.

### Step 4: Access Your Dashboard
After logging in, your candidate dashboard will load. Once the result is published, you will see:
- **Scorecard/Result** option in the left or top navigation menu
- Your **Section-wise scores**
- Your **Composite Score (out of 800)**
- Your **Percentile**

### Step 5: Download Your Scorecard PDF
- Click **"Download Scorecard"**
- Save the PDF on your device
- **Print at least 2–3 copies** — one for your records and one to attach with college applications

---

## Understanding Your MAT PBT Scorecard

Your MAT PBT scorecard contains several important fields:

| Component | Description |
| :--- | :--- |
| **Candidate Name & Roll No.** | Your identity details |
| **Test Mode** | Paper-Based Test (PBT) |
| **Section-wise Raw Scores** | Individual scores for LC, ICR, MS, DA, IGE |
| **Composite Score** | Total out of 800 (LC + ICR + MS + DA scaled) |
| **Percentile** | Your relative performance vs. all test-takers |

> **Note:** The **Indian & Global Environment (IGE)** score appears on your scorecard but is **excluded from the composite score** for merit purposes. Always check individual college criteria, as a few institutes may factor in IGE for shortlisting.

---

## What is a Good MAT PBT Score?

Understanding your score in context helps you target the right colleges:

| Composite Score | Percentile (Approx.) | College Category |
| :--- | :--- | :--- |
| **650–800** | 99+ percentile | Top MAT-accepting B-Schools (TAPMI, JBIMS) |
| **580–650** | 95–99 percentile | Tier-1 MAT colleges (BIMTECH, Great Lakes) |
| **500–580** | 85–95 percentile | Good PGDM colleges (Jaipuria, NDIM, NIBM) |
| **400–500** | 70–85 percentile | Mid-range B-Schools |
| **Below 400** | Below 70 percentile | Consider reappearing in MAT September 2026 |

---

## 🧮 Estimate Your Score — MAT Score Calculator

**Already calculated your answers at home? Don't wait for the official result!**

<div class="calculator-cta-mid">

> 📊 **[Open MAT Score Calculator → /tools/mat-score-calculator](/tools/mat-score-calculator)**
>
> ✅ Enter your **correct answers** and **wrong answers** for LC, MS, DA, and ICR sections  
> ✅ Instantly see your **estimated composite score out of 800**  
> ✅ Get your **expected percentile band** and **top MBA colleges you can target**  
> ✅ Completely free — no signup needed!

</div>

---

## MAT May 2026 Score Validity

Your MAT May 2026 PBT scorecard is valid for **12 months** from the date of declaration. This means you can use it for **MBA/PGDM 2027–29 batch admissions** at all MAT-accepting institutes across India without needing to re-appear.

---

## Top MBA Colleges Accepting MAT PBT Score

Once you have your scorecard, immediately begin applying to relevant colleges:

- **[BIMTECH Greater Noida](/blog/direct-admission-bimtech-greater-noida-management-quota-2026)** – 500+ composite preferred
- **[Jaipuria Institute of Management](/blog/all-about-jaipuria-institute-of-management)** – MAT-friendly admission process
- **[NDIM New Delhi](/blog/all-about-ndim-delhi)** – Direct PGDM admissions with MAT
- **[FOSTIIMA Delhi](/blog/all-about-fostiima-delhi)** – Top Delhi PGDM institute accepting MAT
- **[NIBM Pune](/blog/all-about-nibm-pune)** – Banking & Finance MBA accepting MAT
- **[JIMS Rohini Delhi](/blog/all-about-jims-rohini)** – AICTE-approved PGDM with MAT intake

👉 Read full list: **[MBA Colleges Accepting MAT Score in Delhi NCR 2026](/blog/mba-colleges-accepting-mat-score-delhi-ncr-2026)**

---

## What to Do After Checking Your MAT PBT Score?

### ✅ If Your Score is 80+ Percentile
Act immediately — shortlist colleges, check their GD/PI schedules, and apply directly. Most top MAT-accepting B-Schools run rolling admissions and seats fill fast.

### ✅ If Your Score is 60–80 Percentile
Target mid-range PGDM institutes with strong placement records. Use the **[MAT College Predictor](/tools/mat-college-predictor)** to get a personalised list.

### ✅ If Your Score is Below 60 Percentile
Consider re-appearing in the **MAT September 2026** session. It is the last window for 2027–29 batch admissions at many institutes. Meanwhile, explore institutes accepting **CMAT, MAH-CET, or direct admissions**.

### ✅ Prepare for GD/PI
Once shortlisted, start preparing for Group Discussion and Personal Interview rounds. Read our guide: **[GD-PI Interview Topics & Solutions for MBA](/blog/gdpi-interview-topics-solutions-mba)**

---

## Frequently Asked Questions (FAQs)

**Q1. When will MAT May 2026 PBT result be declared?**  
The MAT May 2026 PBT result (exam date: May 31) is expected between **June 14–20, 2026**.

**Q2. Where can I check my MAT PBT scorecard?**  
Only at the official AIMA portal: **[mat.aima.in](https://mat.aima.in)** → Candidate Login → Dashboard → Scorecard.

**Q3. Is the scorecard sent by email?**  
No. AIMA does not email or post scorecards. You must log in and download it yourself.

**Q4. Can I use my MAT PBT score for CBT-mode colleges?**  
Yes. MAT score is mode-agnostic — colleges accept scores regardless of whether you took PBT, CBT, or IBT.

**Q5. How long is the MAT PBT score valid?**  
Your score is valid for **12 months** from the date of declaration.

**Q6. What is the maximum composite score in MAT?**  
The maximum composite score is **800** (200 per section × 4 sections: LC, ICR, MS, DA).

---

## Related Resources

- **[All About MAT Exam 2026](/blog/all-about-mat-exam)** — Complete exam overview
- **[MAT May 2026 Result Date & Scorecard Download](/blog/mat-may-2026-result-date-scorecard-download)** — All modes result guide
- **[MAT 2026 & CMAT 2026 Colleges Admission Guide](/blog/mat-2026-cmat-2026-colleges-admission-guide)**
- **[MAT College Predictor 2026](/blog/mat-college-predictor-2026-guide)** — Find your best-fit college
- **[MBA Entrance Exam Dates 2026](/blog/mba-entrance-exam-dates-2026)**

---

## 🧮 Final Step: Calculate Your MAT Score Now!

**Don't wait for the official result. Get your estimated score in under 2 minutes.**

<div class="calculator-cta-footer">

> 🚀 **[Open MAT May 2026 Score Calculator — Free & Instant →](/tools/mat-score-calculator)**
>
> Enter your attempted answers for all 4 sections → Get your **composite score**, **percentile estimate**, and a **personalised college list** based on your projected score.  
> **Zero cost. Zero login. Instant results.**

</div>

---

**Need personalised MBA admission guidance?** Connect with **Mohit Jain** for expert counselling. With extensive experience helping students secure admissions into top PGDM and MBA programs using MAT scores, we'll help you identify the best colleges for your profile, score, and budget.

📞 **[Book a Free Counselling Session → careerwithmohit.online](https://www.careerwithmohit.online/inquiry)**
---

### 🚀 Boost Your Preparation

Looking for more resources? **[Explore Our Premium MBA Mock Test Series 2026](/mock-tests)** to get real-time exam experience and detailed performance analytics.

---
