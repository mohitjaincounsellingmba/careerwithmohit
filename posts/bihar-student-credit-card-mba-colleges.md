---
title: "Bihar Student Credit Card (DRCC) MBA Colleges Pan India 2026-27: Complete City-Wise List"
date: "2026-03-12"
description: "Updated city-wise list of top private MBA colleges accepting the Bihar Student Credit Card (DRCC/BSCC) scheme for 2026-27. Learn how to get up to ₹4 Lakhs at 0% interest for MBA in Pune, Delhi, & Bangalore."
keywords: ["Bihar Student Credit Card top MBA colleges 2026", "DRCC approved MBA colleges list", "BSCC scheme 0 percent interest MBA", "MBA colleges for Bihar students in Pune", "Bihar education loan 2026 for PGDM", "DRCC approved colleges Bangalore", "how to apply for BSCC for MBA outside Bihar"]
---

Looking for a more general overview? Read our [**Region-Wise Guide to Top DRCC Colleges**](/blog/top-drcc-approved-mba-colleges).

If you are a student from Bihar planning to pursue an MBA in the 2026-27 session, the **Bihar Student Credit Card (BSCC)** scheme is your most powerful financial tool. Managed through **District Registration & Counselling Centres (DRCC)**, it has recently been updated to offer even better terms for students.

> **Major Update for 2026**: As per the latest government notification, the BSCC loan is now **completely interest-free (0% Interest)** for all categories of students. You only repay what you borrow.

---

### 📑 Table of Contents
1.  [Key Facts (2026-27)](#-bihar-student-credit-card-2026-27--key-facts)
2.  [Eligibility Criteria](#-eligibility-criteria)
3.  [City-Wise College List](#-city-wise-list-of-mba-colleges-accepting-bscc-2026)
4.  [Step-by-Step Application Guide](#-how-to-apply-step-by-step-for-2026)
5.  [Important Tips for DRCC](#-important-drcc-2026-tips)
6.  [FAQs](#-frequently-asked-questions-faq)

---

## 🏦 Bihar Student Credit Card (2026-27) — Key Facts

- **Loan Amount**: Up to **₹4 Lakhs** (Covers tuition, hostel, books, and even a laptop).
- **Interest Rate**: **0% (Interest-Free)** for all students (Male, Female, Transgender).
- **Collateral & Guarantor**: **Not Required** (The Bihar Government acts as your guarantor).
- **Repayment Period**: 
    - Up to 7 years (84 installments) for loans up to ₹2 Lakhs.
    - Up to 10 years (120 installments) for loans above ₹2 Lakhs.
- **Repayment Starts**: 1 year after the course completion or 6 months after securing a job.

[InquiryCard title="Check Your DRCC Eligibility" description="Not sure if your dream college accepts the Bihar Student Credit Card? Get a verified list of BSCC-approved colleges along with admission support." cta="Verify My College Now" type="admission"]

---

## ✅ Eligibility Criteria

- **Domicile**: Permanent resident of Bihar.
- **Age**: Up to **30 years** for Post-Graduate (MBA/PGDM) courses.
- **Academics**: Must have passed 12th standard.
- **Admission**: Must have a valid **Bonafide Certificate** and **Fee Structure** from a recognized institution.

> [!TIP]
> Use our [**MBA ROI Calculator**](/calculator/career-roadmap) to see if the ₹4 Lakh DRCC loan combined with your chosen college's placement will give you a good return on investment.

---

## 🗺️ City-Wise List of MBA Colleges Accepting BSCC (2026)

Students from Bihar have successfully used the DRCC loan for the following institutions. Always confirm the specific course eligibility at your local DRCC office.

### 🏛️ Delhi NCR (Top Rated)
- **[NDIM Delhi](/colleges/ndim-delhi)** — Grade 'A' B-school with excellent CSR placement.
- **[BIMTECH Greater Noida](/colleges/bimtech-greater-noida)** — Highly preferred by Bihar students for PGDM.
- **[FIIB Delhi](/colleges/fiib-delhi)** — Known for International Business and 100% BSCC support.
- **[GL Bajaj Greater Noida](/colleges/gl-bajaj-greater-noida)** — Great ROI (High placement, moderate fees).
- **[Jaipuria Noida](/colleges/jaipuria-noida)** — Centralised placements across 4 campuses.

### 🌿 Pune & Mumbai (The Finance/IT Hub)
- **[SIBM Pune](/colleges/sibm-pune)** — Lavale Campus (Competitive admission).
- **[ITM Navi Mumbai](/colleges/itm-mumbai)** — Specialised in Fintech and Digital Marketing.
- **[ISBM Pune](/colleges/isbm-pune)** — Good and affordable choice for DRCC students.
- **ASM's IBMR** — Chinchwad, Pune.

### 🌺 Bangalore (Silicon Valley of India)
- **[XIME Bangalore](/colleges/xime-bangalore)** — Top-tier placement in Electronics City.
- **[SIBM Bangalore](/colleges/sibm-bangalore)** — Part of Symbiosis International.
- **[ISBR Bangalore](/colleges/isbr-bangalore)** — Awarded for Industry Excellence.
- **[Alliance University](/colleges/alliance-university-bangalore)** — Large campus with diverse networking.

---

## 📋 How to Apply (Step-by-Step for 2026)

1.  **Online Registration**: Fill out the application on the [Official MNSSBY Portal](https://www.7nishchay-yuvaupmission.bihar.gov.in).
2.  **Documents Upload**: Upload digital copies of your Aadhaar, 10th/12th marksheets, and **College Admission Letter**.
3.  **DRCC Visit**: Carry original documents to your district DRCC office for physical verification.
4.  **Disbursement**: Funds are directly transferred to the college bank account semester-wise.

---

## 💡 Important "DRCC 2026" Tips

- **Laptop Allowance**: You can get an additional amount (up to **₹35k**) sanctioned for a laptop if requested in the bonafide.
- **Top-Up Loans**: If your MBA fee is ₹10 Lakhs, DRCC provides ₹4 Lakhs. You can apply for the remaining ₹6 Lakhs through other education loans or use our [**Direct Admission Guide**](/blog/direct-mba-admission-mumbai-2026) to find colleges with lower fees.
- **Course Approval**: Ensure your course is **AICTE/UGC** approved.

---

## ❓ Frequently Asked Questions (FAQ)

### 1. Is the Bihar Student Credit Card really interest-free in 2026?
Yes! For the 2026-27 session, the interest rate is **0% for all categories**. You only repay the principal amount you borrowed.

### 2. Can I apply for DRCC if I am studying outside Bihar?
Yes. The scheme is designed for residents of Bihar to pursue higher education anywhere in India, provided the college/course is approved by the BSCC department.

### 3. What if I don't get a job after my MBA?
You can submit a self-declaration of unemployment to the DRCC. The government provides a grace period and may extend the repayment start date.

### 4. How long does the verification process take?
Usually **45 to 60 days** from the date of your DRCC office visit.

---

[**Get Your DRCC College List & Free Counselling**](/inquiry) | [**Book a Session with Mohit Jain**](/whatsapp)

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "FAQPage",
  "mainEntity": [
    {
      "@type": "Question",
      "name": "Is the Bihar Student Credit Card really interest-free in 2026?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Yes! For the 2026-27 session, the interest rate for the Bihar Student Credit Card (BSCC) is 0% for all categories. You only need to repay the principal amount borrowed."
      }
    },
    {
      "@type": "Question",
      "name": "Can I use DRCC loan for MBA colleges outside Bihar?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Yes, you can use the DRCC loan for recognized and approved MBA colleges across India, including top cities like Pune, Bangalore, Delhi NCR, and Mumbai."
      }
    },
    {
      "@type": "Question",
      "name": "How much loan can I get for an MBA under the Bihar Student Credit Card scheme?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Under the BSCC scheme, students can get a loan of up to ₹4 Lakhs which covers tuition fees, hostel charges, books, and even a laptop."
      }
    }
  ]
}
</script>

