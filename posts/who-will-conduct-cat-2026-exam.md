---
title: 'Who Will Conduct CAT 2026 Exam? Convening IIM, Rotation History & Exam Dates'
date: '2026-07-27'
category: MBA
description: >-
  Discover rankings, direct admission, fees, and placement reports for top
  colleges in Gurgaon, Delhi NCR. Get details on top colleges under GGSIPU, DU,
  and priva
keywords:
  - who will conduct cat 2026 exam
  - which iim will conduct cat 2026
  - cat 2026 convening iim
  - iim indore cat 2026
  - cat exam 2026 conducting body
  - cat 2026 exam date
  - cat convening iim rotation history
  - cat 2026 registration dates
  - iim admission 2026
  - MBA Admissions 2026
  - Delhi NCR Colleges
  - Best Colleges in Delhi NCR
  - Direct Admission Delhi NCR
  - Delhi NCR College Counselling
  - Gurgaon Colleges
  - Best Colleges in Gurgaon
  - Gurgaon Admissions 2026
  - Direct Admission in Gurgaon
  - Top Colleges in Delhi NCR 2026
  - Delhi NCR Direct Admission 2026
  - Colleges in Delhi NCR
  - Delhi NCR Career Counselling
faqs:
  - question: Who is conducting the CAT 2026 exam?
    answer: >-
      The **Indian Institute of Management (IIM) Indore** is the convening and
      conducting body for the **CAT 2026 exam**, following the rotational policy
      among the top six older IIMs.
  - question: What is the expected exam date for CAT 2026?
    answer: >-
      CAT 2026 is scheduled to be conducted on **Sunday, November 29, 2026**
      (the last Sunday of November), across over 170 exam cities in India.
  - question: How are convening IIMs selected for conducting the CAT exam?
    answer: >-
      The responsibility of convening CAT rotates annually among the **six older
      IIMs**: IIM Ahmedabad, IIM Bangalore, IIM Calcutta, IIM Lucknow, IIM
      Kozhikode, and IIM Indore.
  - question: What is the official website for CAT 2026 registration?
    answer: >-
      The official website for CAT 2026 registration, admit card download,
      notifications, and scorecard release is
      **[iimcat.ac.in](https://iimcat.ac.in)**.
location: Delhi NCR
state: Delhi NCR
---

The **Common Admission Test (CAT) 2026** is the most sought-after management entrance examination in India, serving as the primary gateway to the prestigious **Indian Institutes of Management (IIMs)** as well as over 1,200 premier B-schools including [FMS Delhi](/colleges/fms-delhi), SPJIMR, [MDI Gurgaon](/colleges/mdi-gurgaon), and IIT management departments.

Every year, MBA aspirants eagerly wait to know **which IIM will conduct the CAT exam**, as the convening institute often shapes the nuanced flavor, difficulty distribution, and structural presentation of the test paper. 

In this comprehensive guide, we uncover the **convening IIM for CAT 2026**, explore the historical rotation pattern, break down important exam dates, and analyze what candidates can expect from this year's paper.

---

> 🎯 **Benchmark Your MBA Preparation Today!**
>
> [👉 Take Our Free CAT 2026 Full-Length Mock Test & Percentile Predictor](https://www.careerwithmohit.online/tools/cat-mock-test)

---

## 1. Who Will Conduct the CAT 2026 Exam?

The **Indian Institute of Management (IIM) Indore** is the official convening and conducting body for **CAT 2026**.

Each year, the responsibility of hosting and administering the Common Admission Test rotates among the **six older IIMs** (known as the *BLACKI + Indore* core group: [IIM Ahmedabad](/colleges/iim-ahmedabad), [IIM Bangalore](/colleges/iim-bangalore), [IIM Calcutta](/colleges/iim-calcutta), IIM Lucknow, IIM Kozhikode, and IIM Indore). 

Following the established rotation cycle—where [IIM Calcutta](/colleges/iim-calcutta) convened CAT 2024 and IIM Kozhikode convened CAT 2025—**IIM Indore** takes charge of the examination for the 2026–27 admission cycle.

### Key Highlights of CAT 2026 Convening Body
* **Conducting Institute:** Indian Institute of Management (IIM) Indore
* **Expected Convening Convenor:** Appointed Senior Faculty / Admissions Chair, IIM Indore
* **Official CAT Portal:** [iimcat.ac.in](https://iimcat.ac.in)
* **Exam Mode:** Computer-Based Test (CBT) across 3 slots
* **Expected Total Aspirants:** Over 3.2 Lakh candidates

---

## 2. CAT Convening IIM Rotation History (2018 – 2026)

Understanding which IIM convened the exam in previous years helps aspirants trace patterns in paper difficulty and administrative policies. Below is the chronological record of conducting IIMs over recent years:

| Year | Conducting IIM | Exam Date | Major Highlights / Notes |
| :--- | :--- | :--- | :--- |
| **2026** | **IIM Indore** | **November 29, 2026** | **Upcoming exam; focus on balanced QA and analytical VARC** |
| **2025** | IIM Kozhikode | November 30, 2025 | Standard 66-question format across three slots |
| **2024** | [IIM Calcutta](/colleges/iim-calcutta) | November 24, 2024 | Moderate-to-high difficulty Quantitative Aptitude |
| **2023** | IIM Lucknow | November 26, 2023 | High analytical rigor in DILR and QA sections |
| **2022** | [IIM Bangalore](/colleges/iim-bangalore) | November 27, 2022 | Conceptual focus; RC passages from philosophy & sociology |
| **2021** | [IIM Ahmedabad](/colleges/iim-ahmedabad) | November 28, 2021 | Balanced structure; 66 questions (reduced from 76) |
| **2020** | IIM Indore | November 29, 2020 | Transitioned to 2-hour duration due to COVID-19 protocols |
| **2019** | IIM Kozhikode | November 24, 2019 | Standard 3-hour format with 100 questions |
| **2018** | [IIM Calcutta](/colleges/iim-calcutta) | November 25, 2018 | Known for challenging Quantitative Aptitude calculations |

---

## 3. What Are the Roles & Responsibilities of the Convening IIM?

While CAT is a collective entrance test accepted across all 21 IIMs, the **convening IIM** holds exclusive responsibility for managing the end-to-end examination lifecycle:

1. **Official Notification Release:** Publishing the official CAT 2026 advertisement, eligibility criteria, reservation guidelines, and media announcements by **late July 2026**.
2. **Registration & Helpdesk Management:** Operating the registration portal (`iimcat.ac.in`), handling student grievances, application corrections, and test center allocations.
3. **Question Paper & Pattern Design:** Curating the question bank for all three exam slots, ensuring equivalence in testing standards across Verbal Ability & Reading Comprehension (VARC), Data Interpretation & Logical Reasoning (DILR), and Quantitative Aptitude (QA).
4. **Exam Administration:** Coordinating with national testing partners (such as TCS iON) to conduct seamless, tamper-proof Computer-Based Testing across 170+ cities.
5. **Normalization & Result Declaration:** Executing the statistically rigorous equating and normalization process across slots and releasing official percentiles and scorecards by **December 2026 / January 2027**.

---

## 4. CAT 2026 Important Dates & Schedule

Aspirants aiming for IIMs and top B-schools must track the official timeline closely to avoid missing registration or admit card deadlines:

| Event | Official / Expected Date |
| :--- | :--- |
| **Official CAT 2026 Notification** | Late July 2026 |
| **Registration Window Opens** | August 3, 2026 (10:00 AM) |
| **Last Date to Register Online** | September 15, 2026 (5:00 PM) |
| **Application Correction Window** | Late September 2026 |
| **Admit Card Download Begins** | First Week of November 2026 |
| **CAT 2026 Exam Date** | **November 29, 2026 (Sunday)** |
| **Provisional Answer Key & Objection Window** | First Week of December 2026 |
| **CAT 2026 Result Declaration** | Late December 2026 / Early January 2027 |

---

## 5. What to Expect from an IIM Indore Convened CAT Paper?

Historically, when **IIM Indore** convened CAT in **2020** and **2014**, the papers exhibited distinct characteristics that candidates should keep in mind while preparing their mock test strategies:

### A. Verbal Ability & Reading Comprehension (VARC)
* **Passage Diversity:** IIM Indore tends to include well-structured Reading Comprehension passages covering literature, social sciences, humanities, and economics.
* **Inference-Driven Questions:** Expect questions testing author tone, underlying assumptions, and critical inference rather than mere fact-retrieval.
* **Verbal Logic Focus:** Para-jumbles and para-summaries emphasize logical connectivity and syntax flow.

### B. Data Interpretation & Logical Reasoning (DILR)
* **Hybrid LR-DI Sets:** Sets often combine numerical calculations with logical constraints (e.g., matrix-based arrangements, reasoning with percentages or averages).
* **Selection Strategy:** Identifying the two most solvable sets in the first 5 minutes remains crucial for scoring above the 98th percentile.

### C. Quantitative Aptitude (QA)
* **Arithmetic & Algebra Dominance:** Arithmetic (Profit & Loss, Ratios, Time & Work) and Algebra typically constitute nearly **60–65%** of the quant section.
* **Accuracy Over Speed:** While calculations are generally manageable, conceptual clarity is rewarded over rote formula substitution.

---

## 6. Expert Tips to Crack CAT 2026

1. **Start Sectional Timed Practice Early:** Since you cannot switch sections during the 120-minute test, build focus for 40-minute individual bursts.
2. **Analyze Official Past Papers (2018–2025):** Solve previous year CAT question papers to familiarize yourself with how different convening IIMs frame options.
3. **Focus on Quality Mock Tests:** Attempt full-length mock exams every week starting July/August and spend at least double the test duration analyzing your errors.
4. **Balance Academic Profile & Percentile:** Remember that IIM shortlists depend not only on CAT score but also on 10th, 12th, graduation marks, and work experience.

---

## 7. Related MBA & CAT 2026 Guides
* [All About CAT Exam 2026: Syllabus, Eligibility & Pattern](/blog/all-about-cat-exam)
* [10 Proven Tips to Crack CAT 2026: The IIM Topper's Secret](/blog/10-tips-to-crack-cat-exam-2026)
* [All IIM Cut-Offs 2026-28: Admission Criteria & Selection Process](/blog/all-iim-cut-off-2026-28-admission-mba-pgdm)
* [CAT Score Calculator & Percentile Predictor 2026](/tools/cat-score-calculator)
* [Top MBA Colleges in India 2026: Rankings, Fees & Placements](/blog/iims-list-courses-placements-cutoffs-admission)

---

## ❓ Frequently Asked Questions (FAQ)

### Who is conducting the CAT 2026 exam?
The **Indian Institute of Management (IIM) Indore** is the official convening and conducting body for the **CAT 2026 exam**, following the rotational policy among the top six older IIMs.

### What is the expected exam date for CAT 2026?
CAT 2026 is scheduled to be conducted on **Sunday, November 29, 2026** (the last Sunday of November), across over 170 exam cities in India in three separate slots.

### How are convening IIMs selected for conducting the CAT exam?
The responsibility of convening CAT rotates annually among the **six older IIMs**: [IIM Ahmedabad](/colleges/iim-ahmedabad), [IIM Bangalore](/colleges/iim-bangalore), [IIM Calcutta](/colleges/iim-calcutta), IIM Lucknow, IIM Kozhikode, and IIM Indore.

### What is the official website for CAT 2026 registration?
The official website for CAT 2026 registration, admit card download, official notifications, and scorecard download is **[iimcat.ac.in](https://iimcat.ac.in)**.

### Will the syllabus or pattern change for CAT 2026 under IIM Indore?
No major structural deviation is expected. CAT 2026 is expected to retain the 66-question, 120-minute CBT pattern across three sections: VARC (24 questions), DILR (20 questions), and QA (22 questions).

---

### 🚀 Need Personalized Guidance for CAT 2026 & IIM Admissions?

Unsure which B-schools match your profile and target percentile? **[Speak with Our MBA Career Counsellors Today](/inquiry)** or explore our **[Free CAT Mock Tests](/tools/cat-mock-test)** to kickstart your IIM journey!

---

### 🚀 Boost Your Preparation

Looking for more resources? **[Explore Our Premium MBA Mock Test Series 2026](/mock-tests)** to get real-time exam experience and detailed performance analytics.

---
