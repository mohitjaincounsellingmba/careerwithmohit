---
title: "COMEDK UGET Counselling 2026: Dates, Registration & Choice Filling Guide"
date: "2026-03-24"
description: "Everything you need to know about COMEDK UGET Counselling 2026 for B.E./B.Tech admissions in Karnataka's top private engineering colleges. Check registration dates, fees, and top institutes like RVCE & MSRIT."
keywords: ["COMEDK UGET Counselling 2026 dates", "COMEDK registration fee", "RVCE Bangalore cutoff 2026", "COMEDK choice filling guide", "MSRIT Bangalore admission", "COMEDK seat allotment", "BMSCE Bangalore cutoff"]
---

**COMEDK UGET (Consortium of Medical, Engineering and Dental Colleges of Karnataka)** is the centralized admission gateway for engineering seats in over 150+ private unaided engineering colleges in Karnataka. Known for premier institutes like **RVCE Bangalore** and **MSRIT**, COMEDK 2026 is a top-tier choice for students nationwide.

### **COMEDK UGET 2026 Important Dates (Tentative)**
| Event | Expected Date |
| :--- | :--- |
| **Counselling Notification** | June 2026 |
| **Online Registration Starts** | June 2026 |
| **Document Verification** | July 2026 |
| **Choice Filling (Round 1)** | July 2026 |
| **Seat Allotment (Round 1)** | August 2026 |

### **Eligibility & Admission Criteria**
*   **Entrance Exam**: A valid **COMEDK UGET 2026** score (Rank) is mandatory.
*   **Selection**: Admitted through an entirely online centralized counselling process.
*   **Quota**: Open to All India students. Unlike state exams like KCET, COMEDK is one of the biggest private engineering entry points for non-Karnataka students.

### **Counselling Fee & Seat Acceptance**
*   **Counselling Registration Fee**: approx. ₹2,000 - ₹2,500 (Non-refundable).
*   **Initial Part Payment**: ₹55,000 (Mandatory seat acceptance fee after allotment).

### **Top Participating Colleges in COMEDK 2026**
1.  **RV College of Engineering (RVCE), Bangalore** - *Top Choice.*
2.  **M.S. Ramaiah Institute of Technology (MSRIT), Bangalore**.
3.  **B.M.S. College of Engineering (BMSCE), Bangalore**.
4.  **PES University, Bangalore**.
5.  **Dayananda Sagar College of Engineering (DSCE), Bangalore**.
6.  **Siddaganga Institute of Technology (SIT), Tumkur**.
7.  **Bangalore Institute of Technology (BIT), Bangalore**.

### **Step-by-Step COMEDK Online Counselling Procedure**
1.  **Online Registration**: Visit `comedk.org` and register using your application number.
2.  **Document Upload**: Scanned quality copies of 10+2 marks cards, Rank Card, Admit Card, Photo ID, and domicile/category certificates.
3.  **Document Verification**: Once verified by the COMEDK portal, you will be eligible for choice filling. Check your status regularly (Approved/Rejected/Resubmit).
4.  **Mock Allotment**: COMEDK releases a mock list. Use this to gauge your chances and re-order your college choices.
5.  **Final Choice Filling**: Lock your preferences. Put highly-ranked colleges like RVCE and MSRIT at the top!
6.  **Seat Allotment**: If allotted, decide between **Freeze** (Accept), **Float** (Upgrade), or **Reject and Withdraw**.

**Pro Tip for COMEDK 2026**:
Bangalore is the "Silicon Valley" of India. If your rank is good, prioritize colleges in Bangalore (RVCE, MSRIT, BMSCE) as they offer significantly better exposure and high-tech internships compared to remote colleges.

[👉 Check Top COMEDK Bangalore Colleges & Reviews Here!](/colleges?location=Bangalore)

**Stuck with Choice Filling?**
With 150+ colleges, one wrong click can move you to a lower-ranked institute. At **CareerWithMohit**, we provide a professional preference order list based on your COMEDK rank.

[👉 Get Expert COMEDK Counselling Support!](/inquiry)
