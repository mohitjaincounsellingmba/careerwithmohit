...
1.  **Question (Rotational Motion)**: A solid sphere of mass $M$ and radius $R$ rolls without slipping down an inclined plane of height $H$. Find its linear velocity at the bottom.
...
2.  **Question (Electrostatics)**: Two point charges $+q$ and $-q$ are placed at distance $d$. Find the work done in bringing a charge $+Q$ from infinity to the midpoint.
    *   **Solution**: Potential at midpoint due to $+q$ and $-q$ is $V = \frac{kq}{d/2} + \frac{k(-q)}{d/2} = 0$. Work done $W = Q \times \Delta V = Q(0 - 0) = 0$.

3.  **Question (Modern Physics)**: The half-life of a radioactive substance is 10 days. How long will it take for 75% of the substance to decay?
...
1.  **Question (Chemical Kinetics)**: For a first-order reaction $A \rightarrow B$, the rate constant $k$ is $0.693 \, \text{min}^{-1}$. Find the time required for 90% completion.
...
2.  **Question (Coordination Compounds)**: Predict the hybridization and geometry of $[Ni(CN)_4]^{2-}$.
...
3.  **Question (Organic Chemistry)**: What is the major product when Propene reacts with $HBr$ in the presence of Peroxide?
...
1.  **Question (Calculus - Integration)**: Evaluate $\int_{0}^{\pi/2} \frac{\sin x}{\sin x + \cos x} dx$.
...
2.  **Question (Probability)**: If $P(A) = 0.6$, $P(B) = 0.4$, and $P(A \cap B) = 0.2$, are A and B independent?
    *   **Solution**: For independence, $P(A) \times P(B) = P(A \cap B)$. Here $0.6 \times 0.4 = 0.24 \neq 0.2$. Hence, they are **Not Independent**.

3.  **Question (Matrices)**: If $A$ is an idempotent matrix, find $(I+A)^n$.
