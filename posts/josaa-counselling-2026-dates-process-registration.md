---
title: "JoSAA Counselling 2026: Important Dates, Step-by-Step Registration & Choice Filling Process"
date: "2026-03-24"
description: "Everything you need to know about JoSAA 2026 counselling. Get the latest tentative schedule for registration, choice filling, mock allotment, and a complete list of required documents for IIT/NIT admissions."
keywords: ["JoSAA 2026 counselling dates", "JoSAA registration process 2026", "how to fill choices in JoSAA", "JoSAA mock seat allotment 2026", "documents required for JoSAA counselling", "JoSAA freeze float slide explained", "IIT NIT counselling 2026"]
---

The **Joint Seat Allocation Authority (JoSAA)** is the gateway to 110+ premier engineering institutions in India, including all **IITs, NITs, IIITs, and GFTIs**. For candidates who have qualified JEE Main or JEE Advanced 2026, the JoSAA counselling process is the most critical step in securing a seat in their dream college.

With the JoSAA 2026 session expected to begin in **June 2026**, here is a comprehensive guide to the schedule, registration process, and key rules.

### **JoSAA 2026: Important Dates (Tentative)**
The official schedule will be released by the authorities soon. Based on previous trends, here is the expected timeline:

| Event | Tentative Date |
| :--- | :--- |
| **JEE Advanced 2026 Results** | June 1, 2026 |
| **JoSAA Registration Opens** | June 2, 2026 |
| **Choice Filling Window** | June 10 – June 20, 2026 |
| **Mock Seat Allotment 1** | June 15, 2026 |
| **Round 1 Seat Allotment** | June 23, 2026 |
| **Round 2 Allotment** | June 28, 2026 |

### **Step-by-Step JoSAA 2026 Counselling Process**
1.  **Online Registration**: Visit [josaa.nic.in](https://josaa.nic.in) and log in using your JEE Main application number and password.
2.  **Choice Filling**: This is the most vital part. You must list your preferred combinations of Institutes and Branches in decreasing order of preference. 
    *   *Tip: Fill as many choices as possible to avoid losing out in the final rounds.*
3.  **Mock Seat Allotment**: JoSAA releases mock results to show where you might get admission based on your current choices. Use this to re-order your list.
4.  **Seat Allotment Rounds**: JoSAA typically conducts 6 rounds. After each round, if you are allotted a seat, you must choose one of the following:
    *   **Freeze**: You are happy with the seat and want to secure it.
    *   **Float**: You accept the seat but want to wait for a better college/branch in the next round.
    *   **Slide**: You accept the seat but want a better branch within the same institute.
5.  **Fee Payment & Document Upload**: Once you accept a seat, you must pay the **Seat Acceptance Fee (SAF)** and upload documents for online verification.

### **Documents Required for JoSAA 2026**
Ensure you have scanned copies of these documents ready:
*   JEE Main/Advanced Admit Card & Scorecard
*   Class 10th & 12th Marksheets (Date of Birth proof)
*   Category Certificate (OBC-NCL/SC/ST/EWS) - *Must be issued after April 1, 2026*
*   Medical Certificate (in JoSAA format)
*   Cancelled Cheque or Bank Passbook copy
*   Photo ID Proof

### **Seats Available in JoSAA 2026**
Planning your choices requires knowing the seat matrix. Check our detailed institute-wise seat guides:
*   [Total Seats in IITs 2026: JoSAA Seat Matrix](/blog/total-seats-in-iits-2026-seat-matrix)
*   [Total Seats in NITs 2026: State Quota Details](/blog/total-seats-in-nits-2026-seat-matrix)
*   [Total Seats in IIITs 2026: Top Specializations](/blog/total-seats-in-iiits-2026-seat-matrix)
*   [Total Seats in GFTIs 2026: Official Institute List](/blog/total-seats-in-gftis-2026-seat-matrix)

### **Expert Tips for a Successful JoSAA Counselling**
*   **Don't Wait for the Last Date**: Server issues are common on the final day of choice locking.
*   **Check Placement Records**: Before opting for a newer IIIT or GFTI, research their placement trends and campus infrastructure.
*   **Understand CSAB**: If you don't get a seat in JoSAA, you can participate in **CSAB Special Rounds** for vacant seats in NITs/IIITs/GFTIs.

**Need Personalized Counselling Support?**
Choice filling is both an art and a science. Our experts at **CareerWithMohit** have helped thousands of students convert their JEE ranks into seats at the best possible institutes. We provide customized choice-filling lists based on your rank, budget, and location preferences.

[👉 Connect with Our JoSAA Experts Today!](/inquiry)

---

### 🚀 Boost Your Preparation

Looking for more resources? **[Explore Our Premium MBA Mock Test Series 2026](https://www.careerwithmohit.online/tools/mock-tests)** to get real-time exam experience and detailed performance analytics.

---
