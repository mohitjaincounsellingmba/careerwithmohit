---
title: 'NMAT Exam 2026 Last Date of Registration: Deadline, Retake Window, Fees & How to Apply'
date: '2026-09-10'
category: MBA
description: >-
  Check NMAT exam 2026 last date of registration, application deadline, registration fee,
  retake dates, NMIMS cutoff requirements, and step-by-step form fill-up guide.
keywords:
  - nmat exam 2026 last date of registration
  - nmat 2026 registration deadline
  - nmat registration last date
  - nmat by gmac 2026 application form
  - nmat 2026 exam dates
  - nmat registration fee 2026
  - nmims registration last date 2026
  - how to apply for nmat 2026
faqs:
  - question: What is the NMAT exam 2026 last date of registration?
    answer: >-
      The regular registration window for NMAT by GMAC 2026 closes in the second week of
      October 2026 (expected October 10 to October 13, 2026, at 11:59 PM). Retake registrations
      remain open until mid-December 2026.
  - question: What is the registration fee for NMAT 2026?
    answer: >-
      The registration fee for the main exam is ₹3,000 + applicable taxes (18% GST). The retake fee
      is also ₹3,000 + taxes per attempt, and exam rescheduling charges are ₹1,200 + taxes.
  - question: Can I take NMAT multiple times in 2026?
    answer: >-
      Yes, candidates are permitted up to three attempts (1 main attempt + 2 retakes) within
      the testing window. However, please note that NMIMS Mumbai considers only the first (main) attempt
      score for its flagship MBA programs.
  - question: Do I need to apply separately for NMIMS Mumbai?
    answer: >-
      Yes! Registering on the NMAT by GMAC portal only enters you for the exam. You must separately
      fill out the NMIMS MBA application form on `nmims.edu` before its deadline (typically the same
      date as NMAT regular registration).
  - question: What is the exam pattern for NMAT 2026?
    answer: >-
      NMAT is a computer-adaptive test with 108 questions across three sections (Language Skills,
      Quantitative Skills, and Logical Reasoning) to be completed in 120 minutes. There is no negative marking.
location: All India
state: India
---

# NMAT Exam 2026 Last Date of Registration: Deadline, Fees & NMIMS Application Guide

> 💡 **Key Takeaways (Direct AI Answer Summary)**
> - **Main Registration Deadline:** NMAT by GMAC 2026 regular registration opens on August 1, 2026, and closes in the second week of October 2026 (expected around October 10–13, 2026).
> - **Application Fee & Retakes:** ₹3,000 + 18% GST per attempt. Candidates can appear up to 3 times (1 main + 2 retakes), with retake windows open until mid-December 2026.
> - **Dual Registration Rule:** You must register separately on both `register.nmat.org` (for the test slot) and individual college portals (especially [NMIMS Mumbai](/colleges/nmims-mumbai) at `nmims.edu`).

---

The **NMAT by GMAC** (Graduate Management Admission Council) is one of India's most student-friendly and prestigious management entrance examinations. It is the premier pathway to the coveted **SVKM's Narsee Monjee [Institute of Management Studies](/colleges/institute-of-management-studies) (NMIMS)** campuses in Mumbai, Bengaluru, Navi Mumbai, Hyderabad, and Indore, along with esteemed institutions like K J Somaiya, TAPMI, XIM University Bhubaneswar, and SDA Bocconi Asia Center.

Unlike single-day tests like CAT or XAT, NMAT provides a 75-day testing window with customizable sectional order and zero negative marking. However, adhering to the **NMAT exam 2026 last date of registration** is vital because missing the regular window blocks you from taking the main exam and eliminates your eligibility for top NMIMS programs.

---

> 🎯 **Aiming for 235+ in NMAT 2026?**
>
> [👉 Practice Free MBA Mock Tests Now](/mock-tests) | [Read All About NMAT Exam Pattern & Syllabus](/blog/all-about-nmat-exam)

---

## NMAT 2026 Important Registration & Exam Dates

Below is the verified timeline for NMAT by GMAC 2026 admissions cycle:

| Event Milestone | Expected Official Dates | Crucial Instructions |
| :--- | :--- | :--- |
| **Official Notification & Website Live** | Late July 2026 | GMAC releases details on `register.nmat.org` |
| **Regular Registration Starts** | August 1, 2026 | Create candidate profile and pay test fee |
| **Exam Scheduling Window Starts** | August 1, 2026 | Book preferred date, slot, and test centre |
| **NMAT 2026 Last Date of Registration** | **October 10 – October 13, 2026** | Portal closes for main attempt registration |
| **Exam Delivery Window (Test Period)** | October 10 – December 18, 2026 | Test conducted over ~75 days across India |
| **Retake Registration Window** | Mid-October – Mid-December 2026 | Can register only after completing previous attempt |
| **NMIMS Mumbai Separate Application Last Date** | Second Week of October 2026 | Must apply prior to or alongside NMAT form |
| **Scorecard Download Window** | Within 48–72 hours after exam | Scaled scorecard available for download |

---

## NMAT 2026 Registration Fees & Charge Breakdown

GMAC charges fixed fees for testing, retakes, and rescheduling. All payments are collected online:

* **Regular Registration (Main Attempt):** ₹3,000 + 18% GST (Approx. ₹3,540)
* **Retake Registration (Per Attempt, Max 2 Retakes):** ₹3,000 + 18% GST (Approx. ₹3,540)
* **Exam Rescheduling Fee:** ₹1,200 + 18% GST (Approx. ₹1,416)
* **Additional Score Reports (Beyond first 5 included B-schools):** ₹300 + 18% GST per school

*Note: Your NMAT registration fee includes sending your official scaled scores to up to 5 participating programmes or universities for free.*

---

## Eligibility Criteria for NMAT 2026

* **Graduation Degree:** Minimum 3-year bachelor’s degree in any discipline from a recognized university.
* **Minimum Aggregate Marks:** At least **50% aggregate marks** in graduation (or equivalent CGPA).
* **Final Year Candidates:** Final-year undergraduate students are eligible to apply, provided they submit proof of passing with 50%+ marks at the time of final admission.
* **NMIMS Specific Criteria:** For NMIMS Mumbai Flagship MBA (Core / HR), 50% in graduation with mathematics/statistics at 10+2 or graduation level is preferred.

---

## Documents Required for NMAT 2026 Form Fill-Up

Keep the following documents scanned and ready before starting the online form:

1. **Scanned Photograph:** Recent passport-size photograph with white/light background (JPEG/PNG format, size under 1 MB).
2. **Valid Photo ID Proof:** Original Government ID (Passport, PAN Card, Voter ID, or Aadhaar Card with photo) matching your registered name exactly.
3. **Academic Records:** 10th, 12th, and Graduation marksheets for accurate score entry.
4. **Work Experience Certificate (If any):** Exact duration in months and details of employer.
5. **Credit Card / Debit Card / Net Banking / UPI:** For smooth digital fee payment.

---

## Step-by-Step Guide: How to Apply for NMAT 2026

Follow these steps carefully before the registration last date:

### 1. Account Creation on `register.nmat.org`
* Go to the official GMAC registration page.
* Provide your name, primary email address, mobile number, and select country of residence.
* Verify your account via the activation link sent to your email.

### 2. Enter Personal, Contact & Educational Details
* Fill in your parents' names, date of birth, identity card details, and residential address.
* Enter your Class 10, Class 12, and undergraduate degree percentage and university details.

### 3. Select 5 Participating B-Schools (Free)
* Under the B-Schools Selection section, choose 5 B-school programs that you want to send your scores to without any extra charge (e.g., NMIMS Mumbai, TAPMI, K J Somaiya, XIM University, SDA Bocconi).
* If you wish to apply to more than 5 institutes, you can add them by paying ₹300 + GST each.

### 4. Pay the Registration Fee
* Proceed to the secure checkout page and complete payment using credit/debit card, UPI, or net banking.
* Once the payment is verified, your registration dashboard will be unlocked.

### 5. Schedule Your Exam Date, Slot & Venue
* Under the "Schedule Exam" tab, choose your preferred testing mode: **Test Centre Based Exam** or **Online Proctored Exam at Home**.
* Select your preferred test city, examination centre, test date, and available time slot (Morning or Afternoon).

---

## Top B-Schools Accepting NMAT 2026 Scores & Expected Cutoffs

| College Name | Total Fees (2027–29) | Avg Package (Latest) | ROI & Admission Eligibility |
| :--- | :--- | :--- | :--- |
| **NMIMS Mumbai (SBM)** | ₹24.00 Lakhs | ₹26.63 LPA | NMAT 232+ Scaled Score · Tier-1 Private Flagship |
| **NMIMS Bengaluru / Navi Mumbai** | ₹20.00 Lakhs | ₹14.00 LPA | NMAT 215-225 Score · Rapidly Growing Tech Campuses |
| **[K J Somaiya Institute of Management](/colleges/kj-somaiya-mumbai)** | ₹20.80 Lakhs | ₹12.50 LPA | NMAT 222+ / CAT / XAT · Strong Financial & HR Network |
| **TAPMI Manipal (Select Programs)** | ₹17.50 Lakhs | ₹14.80 LPA | NMAT 210-220 Score · AACSB Accredited |
| **XIM University (School of HRM)** | ₹21.00 Lakhs | ₹16.64 LPA | NMAT 210+ / XAT / CAT · Top HR Specialization in India |
| **SDA Bocconi Asia Center** | ₹20.50 Lakhs | ₹14.50 LPA | NMAT / CAT / GMAT · International Dual Degree & Milan Term |
| **ISBR Bangalore** | ₹10.50 Lakhs | ₹8.20 LPA | NMAT / MAT / CMAT · Top Industry Placement Ecosystem |

---

## Critical Rules Candidates Must Know Before Registering

1. **The NMIMS First Attempt Rule:** SVKM's NMIMS explicitly states that for admissions to its flagship MBA programs at the Mumbai campus, **only the candidate's first attempt (main score)** is accepted. Retake scores are not considered for NMIMS Mumbai shortlist.
2. **Apply Separately to Colleges:** Registering on the GMAC portal does not mean you have applied to NMIMS or K J Somaiya. You must fill out the college-specific application on their individual websites before their respective deadlines.
3. **Choice of Section Sequence:** In NMAT, you can decide the order of the three sections (e.g., Language Skills -> Logical Reasoning -> Quantitative Skills). Choose the sequence that matches your mock test strategy.
4. **Rescheduling Deadline:** You can reschedule your test date up to 72 hours before your scheduled appointment time by paying the ₹1,200 rescheduling fee.

---

## Frequently Asked Questions (FAQs)

### What is the NMAT exam 2026 last date of registration?
The regular registration window for NMAT 2026 closes in the second week of October 2026 (expected between October 10 and October 13, 2026). Retake registration remains open until mid-December 2026.

### How much is the NMAT 2026 registration fee?
The test registration fee is ₹3,000 + 18% GST (total approx. ₹3,540) per attempt.

### Does NMAT have negative marking?
No. NMAT does not have negative marking. Candidates are encouraged to attempt all 108 questions.

### Can I change my NMAT test centre after booking?
Yes, you can reschedule your exam date or test centre up to 72 hours prior to your scheduled slot by paying a rescheduling fee of ₹1,200 + taxes.

---

### Related Articles & Useful Resources
* [All About NMAT Exam: Pattern, Syllabus & NMIMS Cutoffs](/blog/all-about-nmat-exam)
* [All About NMIMS Campuses: Fees, Cutoffs & Placement Reports](/blog/all-about-nmims-campuses)
* [CAT Exam 2026 Last Date of Registration & Timeline](/blog/cat-exam-2026-last-date-of-registration)
* [Top MBA Entrance Exams 2026 Comprehensive Guide](/blog/all-about-omets-mba-entrance-exams-2026)
* [Free MBA Preparation & Counselling Consultation](/inquiry)

---

### 🚀 Boost Your Preparation

Looking for more resources? **[Explore Our Premium MBA Mock Test Series 2026](/mock-tests)** to get real-time exam experience and detailed performance analytics.

---
