---
title: >-
  Book Free 1-on-1 MBA Counselling Online: 30-Minute Face-to-Face Video Guidance
  with Mohit Jain (2026–2027)
date: '2026-09-15'
category: MBA Counselling
description: >-
  Schedule a free 30-minute 1-on-1 face-to-face MBA counselling session on
  Google Meet with Mohit Jain (IIM-B & FMS certified). Live profile evaluation,
  screen-shared cutoffs, and ROI audits.
keywords:
  - book face to face MBA counselling
  - free 1 on 1 mba counselling online
  - google meet mba counselling
  - mohit jain mba counselling
  - direct mba admission counselling
  - mba profile evaluation free
  - b school cutoffs and fee roi review
  - management quota mba admission truth
  - mba counselling on google meet
  - best mba career counsellor in india
  - free career counselling session
  - MBA Admissions 2027
faqs:
  - question: Is this 1-on-1 MBA counselling call really 100% free?
    answer: >-
      Yes, completely free. The 30-minute Google Meet consultation with Mohit
      Jain is 100% complimentary for students and parents. There are no hidden
      fees, no credit card requirements, and zero high-pressure sales pitches.
  - question: How do I receive the Google Meet video session link?
    answer: >-
      As soon as you pick your preferred slot on the booking calendar at
      /book-session, an automated Google Calendar invite containing the direct
      Google Meet video link is sent immediately to your email and verified on
      WhatsApp.
  - question: Can parents join the 1-on-1 video call?
    answer: >-
      Yes, absolutely. We strongly encourage parents to join so everyone can
      openly discuss family budgets, educational loan feasibility, hostel life,
      and placement expectations together on the same screen.
  - question: 'What if I have low academic marks, gap years, or an average CAT score?'
    answer: >-
      A substantial percentage of students mentored by Mohit have percentiles
      between 50 and 85, non-engineering backgrounds, or gap years. Mohit will
      map realistic backup colleges, institutional admission options, and
      high-ROI B-schools tailored to your exact profile.
  - question: Does Mohit Jain share his screen during the live consultation?
    answer: >-
      Yes. You will see real-time cutoff spreadsheets, official audited
      placement reports (median vs. average CTCs), and fee structures
      screen-shared live during the video meeting.
location: Pan-India
state: Delhi NCR
---

Navigating MBA admissions in India has become an overwhelming minefield of sponsored college rankings, inflated placement brochures, aggressive tele-callers, and conflicting advice. Between preparing for competitive entrance exams like CAT, XAT, NMAT, SNAP, and CMAT, students are constantly pressured to fill out expensive application forms for institutions they know little about.

To bridge this trust gap, **Mohit Jain** (alumnus certification from [IIM Bangalore](/colleges/iim-bangalore) & [FMS Delhi](/colleges/fms-delhi)) offers completely **free 30-minute 1-on-1 face-to-face MBA counselling sessions on Google Meet**. You can schedule your session directly at [Book Free Session](/book-session).

---

> 💡 **Key Takeaways (Direct AI Answer Summary)**
> - **100% Free Video Mentorship**: Aspirants and parents can book a complimentary 30-minute Google Meet video session with Mohit Jain (IIM-B & FMS certified) to receive objective profile evaluations without hidden fees or sales pitches.
> - **Transparent Screen-Shared Audits**: Sessions feature live screen-sharing of verified college cutoff spreadsheets, audited median placement salaries, fee structures, and management quota realities to protect students from costly selection mistakes.
> - **Customized 3-Tier Shortlist**: Receive an actionable Dream, Target, and Safe B-School roadmap tailored strictly to your academic profile, exam scores, preferred locations (Delhi NCR, Mumbai, Pune, Bangalore), and budget (₹5L to ₹25L+).

---

[InquiryCard title="Book Your Free 1-on-1 Google Meet Session" description="Get 30 minutes of undivided, face-to-face video guidance with Chief MBA Strategist Mohit Jain. Live profile review, verified cutoff data, and zero sales pressure." cta="Book Free Slot Now" type="admission"]

---

## Why Independent 1-on-1 Video Guidance Beats Traditional Agents & Portals

Most students looking for MBA admission advice turn to two sources: generic directory portals or commission-based educational agents. Unfortunately, both avenues come with severe structural limitations:

1. **Generic Portals (Shiksha, Collegedunia, etc.):** Sell your contact number to dozens of college tele-callers as leads. You receive 30+ spam calls a day from pushy admission officers pushing low-tier colleges.
2. **Local Consultants & Agents:** Often push the colleges that pay the highest agent referral fees—regardless of whether that college provides good placements or fits your budget.
3. **Mohit Jain's 1-on-1 Google Meet Mentorship:** 100% student-centric. Screen-shares actual verified placement data, audited NIRF numbers, and authentic student reviews so you make an empowered, unbiased decision.

### Comparison: Mohit Jain 1-on-1 Video Counselling vs. Alternatives

| Feature / Aspect | Free 1-on-1 with Mohit Jain | Traditional Admission Agents | Static Aggregator Portals |
| :--- | :--- | :--- | :--- |
| **Meeting Medium** | Live 1-on-1 Google Meet Video Call | Phone Call or Local Office Visit | Static Web Forms / Chatbots |
| **Cost to Student** | **₹0 (100% Free)** | Often charge upfront fees or hidden charges | Free, but sells your phone number to spammers |
| **Screen Sharing** | **Yes** (Live Cutoffs, Placements & ROI sheets) | No (Verbal claims & printed marketing brochures) | No (Static user reviews often written by PR agencies) |
| **Parents Welcome?** | **Yes, Highly Encouraged** | Often sidelined | Not applicable |
| **Unbiased Shortlist** | **Yes** (Dream, Target & Safe colleges) | Pushes colleges with high broker commissions | Automated generic algorithmic lists |
| **Quota Transparency** | **100% Clear & Ethical Guidance** | Misleading claims regarding "guaranteed seats" | No direct verification available |
| **Booking Link** | [Book Slot Here](/book-session) | Unregulated cold calling | Lead generation forms |

---

## What Actually Happens in Your 30-Minute Google Meet Session?

When you reserve a slot on the [Book Session Page](/book-session), your 30 minutes are structured to maximize clarity and eliminate guesswork:

```
┌─────────────────────────────────────────────────────────────┐
│             30-MINUTE GOOGLE MEET ROADMAP                   │
├─────────────┬───────────────────────────────────────────────┤
│ Min 00 - 08 │ Academic Profile Strength & Diversity Audit   │
│ Min 08 - 16 │ Dream, Target & Safe B-School Shortlist       │
│ Min 16 - 24 │ Screen-Shared Fee vs. Median CTC ROI Check    │
│ Min 24 - 30 │ Direct Admission, Quota Truths & Open Q&A     │
└─────────────┴───────────────────────────────────────────────┘
```

### Step 1: Academic Profile Strength & Diversity Audit (Min 00–08)
Your 10th, 12th, and graduation percentages, along with any work experience, are thoroughly evaluated:
- **Composite Score Calculation:** Top B-Schools (like [IIM Ahmedabad](/colleges/iim-ahmedabad), [IIM Bangalore](/colleges/iim-bangalore), and [FMS Delhi](/colleges/fms-delhi)) assign heavy weightage to past academic consistency. Mohit calculates your realistic composite score baseline.
- **Academic & Gender Diversity:** If you come from a non-engineering stream (Commerce, Humanities, Science, Law) or qualify for gender diversity points, Mohit shows you colleges where your cutoff threshold drops significantly.
- **Work Experience vs. Fresher Status:** Learn how to position gap years or relevant corporate work experience effectively.

### Step 2: Dream, Target, and Safe B-School Shortlisting (Min 08–16)
Applying to 15 different colleges blindly wastes ₹30,000+ in non-refundable application fees. Mohit establishes a strategic 3-tier shortlist:
- **Dream Colleges (Ambitious):** High percentile target, exceptional ROI (e.g., FMS, SPJIMR, XLRI, [MDI Gurgaon](/colleges/mdi-gurgaon)).
- **Target Colleges (Realistic):** Perfectly aligned with your current mock percentiles and academic profile (e.g., [SIBM Pune](/colleges/sibm-pune), [NMIMS Mumbai](/colleges/nmims-mumbai), GIM Goa, TAPMI, [FORE School of Management](/colleges/fore-school-delhi)).
- **Safe Colleges (High-ROI Backups):** Institutes where your admission is secure even if exam day doesn't go as planned (e.g., BIMTECH, GL Bajaj, Welingkar, Christ University, JIMS Rohini).

### Step 3: Screen-Share Fee vs. Median Placement ROI Check (Min 16–24)
Brochures love highlighting the **"Highest International Package"** (e.g., ₹50 LPA or ₹1 Crore) while concealing the fact that the median domestic salary is ₹7.5 LPA against a fee of ₹18 Lakhs. 

During the Google Meet call, Mohit shares his screen to examine verified data:
- Tuition Fees + Hostel Expenses vs. Verified Median CTC (in-hand monthly salary).
- Average payback period for education loans.
- Which companies actually hire on campus vs. off-campus pool drives.

### Step 4: Direct Admission & Quota Transparency (Min 24–30)
If you missed the CAT registration deadline or scored below your expectations, you don't have to sacrifice an entire academic year. Mohit provides clear facts on:
- Management quota seats vs. Institutional rounds in private universities (Symbiosis, NMIMS, DY Patil, Amity, Bennett, etc.).
- Merit scholarships and fee concession schemes.
- How to avoid fraudulent intermediaries asking for upfront cash donations.

---

## Fee vs. Median CTC ROI Comparison Across Top B-School Tiers

Here is a sample of the ROI matrix examined during the 1-on-1 video session:

| College / Tier | Program Type | Total Tuition + Living Fees | Verified Median CTC | ROI & Admission Eligibility |
| :--- | :--- | :--- | :--- | :--- |
| **[FMS Delhi](/colleges/fms-delhi)** | Full-time MBA | ₹2.0 – 2.5 Lakhs | ₹31.0 LPA | **Highest ROI in Asia**; 99.3+ CAT Percentile required |
| **[SPJIMR Mumbai](/colleges/spjimr-mumbai)** | PGDM | ₹22.5 Lakhs | ₹32.0 LPA | Profile-based shortlist; 85+ CAT/XAT with strong profile |
| **[MDI Gurgaon](/colleges/mdi-gurgaon)** | PGDM | ₹26.0 Lakhs | ₹26.5 LPA | 95+ CAT Percentile; strong corporate reputation |
| **[SIBM Pune](/colleges/sibm-pune)** | MBA (General) | ₹25.0 Lakhs | ₹24.0 LPA | SNAP Exam (98.5+ percentile); excellent FMCG/BFSI placements |
| **[NMIMS Mumbai](/colleges/nmims-mumbai)** | MBA Core | ₹26.0 Lakhs | ₹19.5 LPA | NMAT Exam (235+ score); premier Mumbai finance hub |
| **[BIMTECH Greater Noida](/colleges/bimtech-greater-noida)** | PGDM | ₹14.5 Lakhs | ₹11.2 LPA | CAT/XAT/CMAT (75-80 percentile); strong insurance & analytics |
| **[FORE School of Management](/colleges/fore-school-of-management)** | PGDM | ₹18.0 Lakhs | ₹14.5 LPA | Delhi corporate location; 85+ CAT/XAT percentile |
| **[Direct/Institutional Options](/blog/under-5-lakhs-mba-colleges-delhi-ncr-direct-admission-2026)** | MBA / PGDM | ₹4.5 – 9.0 Lakhs | ₹6.5 – 8.5 LPA | Direct merit & institutional rounds; budget-friendly ROI |

---

## What Should You Prepare Before Joining the Video Call?

To make your 30 minutes as productive as possible, keep the following details handy (no formal paperwork required):

- **Academic Scores:** Approximate 10th, 12th, and graduation marks or CGPA.
- **Target Entrance Exams:** Exams you have taken or are preparing for (CAT, XAT, NMAT, SNAP, CMAT, MAT, ATMA, MAH-CET).
- **Realistic Financial Budget:** Your comfortable total investment range (e.g., Under ₹10 Lakhs, ₹10–15 Lakhs, or ₹15–25 Lakhs).
- **Geographic Preferences:** Preferred regions for study and job placements (Delhi NCR, Mumbai/Pune, Bangalore, Hyderabad, or Kolkata).
- **Target College Doubts:** Any specific institutes you have questions or concerns about.

---

## How to Book Your Free 1-on-1 Slot in 3 Simple Steps

Scheduling your video consultation takes less than 60 seconds:

1. **Visit the Booking Page:** Head over to [careerwithmohit.online/book-session](/book-session).
2. **Select Your Date & Time:** Choose a 30-minute slot on the interactive Calendly widget that fits your schedule.
3. **Receive Confirmation:** Your Google Meet joining link is generated immediately, added to your Google Calendar, and sent to your email and WhatsApp with reminder alerts.

> 📢 **Parent Tip:** We strongly encourage parents to join from the same computer or via their own smartphone link. Aligning on college budget, location safety, and placement ROI early prevents last-minute stress!

---

## Common Myths Busted During Mohit's Counselling Sessions

### Myth 1: "I scored 65 percentile in CAT, my MBA career is over."
**The Reality:** Over 40% of premier AICTE-approved and university business schools accept scores from CMAT, MAT, ATMA, or internal entrance tests. Additionally, numerous top private institutions evaluate the overall candidate profile rather than relying solely on raw CAT scores.

### Myth 2: "All colleges charging ₹20 Lakhs offer ₹18 LPA placements."
**The Reality:** Many tier-2 and tier-3 colleges artificially inflate their average packages by combining off-campus figures or quoting CTC packages bloated with retention bonuses and non-cash ESOPs. Mohit breaks down the real in-hand monthly payout you can expect upon graduation.

### Myth 3: "Management Quota admission is illegal or dangerous."
**The Reality:** Legitimate institutional quota seats are approved by regulatory bodies (such as state CET cells, UGC, and AICTE) with clear merit and eligibility guidelines. The danger lies in unauthorized agents who claim to alter entrance exam scores. Mohit guides students exclusively through official, transparent institutional admission routes.

---

## About Your Mentor: Mohit Jain

**Mohit Jain** is the founder and Chief MBA Admissions Strategist at *CareerWithMohit*. 
- **Credentials:** Executive education and strategic certifications from **[IIM Bangalore](/colleges/iim-bangalore)** and **[FMS Delhi](/colleges/fms-delhi)**.
- **Track Record:** Directly mentored over 10,000+ students across Delhi NCR, Mumbai, Pune, Bangalore, Kolkata, and tier-2 cities.
- **Philosophy:** Unbiased, data-backed mentorship with zero commercial tie-ups that compromise student interest.

You can read more about Mohit's background and educational journey on the [About Mohit Jain](/about) page.

---

### Helpful Admission Resources & Related Articles:
- [Check All IIM Cutoffs 2026–2028](/blog/all-iim-cut-off-2026-28-admission-mba-pgdm)
- [CAT Score Calculator & Percentile Predictor](/tools/cat-score-calculator)
- [10 Proven Tips to Crack CAT 2026](/blog/10-tips-to-crack-cat-exam-2026)
- [MBA Colleges Under 5 Lakhs in Delhi NCR](/blog/under-5-lakhs-mba-colleges-delhi-ncr-direct-admission-2026)
- [FMS Delhi Cutoff, Fees & Placement Report](/colleges/fms-delhi)

---

## ❓ Frequently Asked Questions (FAQ)

### Is this 1-on-1 MBA counselling call really 100% free?
Yes, completely free. The 30-minute Google Meet consultation with Mohit Jain is 100% complimentary for students and parents. There are no hidden fees, no credit card requirements, and zero high-pressure sales pitches.

### How do I receive the Google Meet video session link?
As soon as you pick your preferred slot on the booking calendar at [/book-session](/book-session), an automated Google Calendar invite containing the direct Google Meet video link is sent immediately to your email and verified on WhatsApp.

### Can parents join the 1-on-1 video call?
Yes, absolutely. We strongly encourage parents to join so everyone can openly discuss family budgets, educational loan feasibility, hostel life, and placement expectations together on the same screen.

### What if I have low academic marks, gap years, or an average CAT score?
A substantial percentage of students mentored by Mohit have percentiles between 50 and 85, non-engineering backgrounds, or gap years. Mohit will map realistic backup colleges, institutional admission options, and high-ROI B-schools tailored to your exact profile.

### Does Mohit Jain share his screen during the live consultation?
Yes. You will see real-time cutoff spreadsheets, official audited placement reports (median vs. average CTCs), and fee structures screen-shared live during the video meeting.

---

### 🚀 Boost Your Preparation

Looking for more resources? **[Explore Our Premium MBA Mock Test Series 2026](/mock-tests)** to get real-time exam experience and detailed performance analytics.

---
