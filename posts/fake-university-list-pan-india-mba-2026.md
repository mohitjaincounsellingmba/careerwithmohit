...
keywords:
  ...
  - aicte unapproved mba colleges
  - fake mba colleges in delhi ncr
  - fake mba institutes bangalore
...
```mermaid
graph TD
    A[UGC Fake Universities List] --> B[Delhi NCR: 12 Institutions]
    A --> C[Uttar Pradesh: 4 Institutions]
    A --> D[West Bengal: 2 Institutions]
    A --> E[Karnataka: 2 Institutions]
    A --> F[Maharashtra: 2 Institutions]
    A --> G[Other States: 10 Institutions]
```
...
```mermaid
graph LR
    A[MBA Admission Scams] --> B[Scam 1: Standalone MBA Degree]
    A --> C[Scam 2: Non-AICTE Autonomous PGDM]
    A --> D[Scam 3: Fake Foreign Online Degrees]
    A --> E[Scam 4: Banned Study Centers]
    A --> F[Scam 5: Fabricated Accreditations]
```
...
sequenceDiagram
    autonumber
    actor Student
    participant UGC as UGC Portal (ugc.gov.in)
    participant AICTE as AICTE Portal (aicte-india.org)
    participant AIU as AIU Equivalence (aiu.ac.in)
    participant DEB as UGC-DEB (deb.ugc.ac.in)

    Student->>UGC: Step 1: Check University Status (Sec 2f & 3)
    Student->>AICTE: Step 2: Verify Extension of Approval (EoA)
    Student->>AIU: Step 3: Check PGDM-to-MBA Equivalence
    Student->>DEB: Step 4: Verify Online/Distance ODL Approval
```
