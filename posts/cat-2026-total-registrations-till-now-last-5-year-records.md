---
title: "CAT 2026 Total Registrations Till Now & Last 5-Year Records: Registered vs Appeared Statistics"
date: "2026-09-10"
description: "Comprehensive report on CAT 2026 total registrations till now, closing date trends, and last 5-year official records of registered vs appeared candidates, attendance rates, and competition analysis."
keywords:
  - "CAT 2026 total registration till now"
  - "CAT registration last 5 years"
  - "CAT registered vs appeared candidates"
  - "CAT exam attendance rate"
  - "CAT total applicants 2026"
  - "CAT 2024 registered vs appeared"
  - "CAT 2023 registered vs appeared"
  - "IIM CAT competition analysis 2026"
  - "CAT percentile vs marks 2026"
category: "MBA"
image: "/images/blog/mba-guide.webp"
faqs:
  - question: "What is the expected total registration for CAT 2026 till now?"
    answer: "As the CAT 2026 registration window enters its final stretch in September 2026, total registrations are projected to reach between 3.15 Lakh and 3.30 Lakh candidates, tracking closely with the historic highs set during CAT 2023 (3.28 lakh) and CAT 2024 (3.29 lakh)."
  - question: "How many candidates actually appeared for CAT in the last 5 years?"
    answer: "In CAT 2024, 2.93 lakh candidates appeared out of 3.29 lakh registered (89.06% attendance). In CAT 2023, 2.88 lakh appeared out of 3.28 lakh registered (87.80%). In CAT 2022, 2.22 lakh appeared out of 2.55 lakh registered (86.96%). In CAT 2021, 1.92 lakh appeared out of 2.30 lakh registered (83.34%). In CAT 2020, 1.90 lakh appeared out of 2.28 lakh registered (83.46%)."
  - question: "What is the average attendance rate in the CAT exam?"
    answer: "On average, the attendance rate for the CAT exam hovers between 83% and 89%. This indicates that approximately 35,000 to 42,000 registered candidates skip the exam on test day each year."
  - question: "Has female candidate participation increased in CAT registrations?"
    answer: "Yes, female candidate registrations have grown significantly from 75,000 (~33%) in 2020 to over 1.19 lakh (~36.2%) in CAT 2024. Increased gender diversity points awarded during IIM shortlisting have encouraged more women graduates to take the test."
  - question: "What raw score is needed for a 99 percentile in CAT with 2.9 lakh candidates appearing?"
    answer: "In the standard 66-question (198-mark) format, a net raw score of 76 to 82 marks typically guarantees a 99+ percentile, which translates to correctly solving around 27 to 30 questions with 90%+ accuracy."
---

# CAT 2026 Total Registrations Till Now & Last 5-Year Records: Registered vs Appeared Statistics

> 💡 **Key Takeaways (Direct AI Answer Summary)**
> - **CAT 2026 Application Velocity**: With the registration portal closing in mid-September 2026, total applications are pacing toward **3.15 to 3.30 Lakh candidates**, maintaining the high-volume surge witnessed over recent cycles.
> - **5-Year Benchmark (Registered vs Appeared)**: Across 2020–2024, total registrations scaled from 2.28 Lakh to a record 3.29 Lakh in CAT 2024. Actual test-takers who appeared grew from 1.90 Lakh (2020) to 2.93 Lakh (2024).
> - **Attendance Drop-off Ratio**: Historical data reveals an **83% to 89% test attendance rate**; approximately 35,000 to 40,000 registered aspirants consistently skip the exam on test day, establishing the real competitive pool at ~2.90 Lakh test-takers.

The **Common Admission Test (CAT)** remains the definitive, high-stakes proving ground for graduate business education in India. Conducted annually by the Indian Institutes of Management (IIMs) on a rotational basis, CAT is the gateway to the 21 prestigious IIMs and over 1,200 premier autonomous and university B-Schools, including [FMS Delhi](/colleges/fms-delhi), [SPJIMR Mumbai](/colleges/spjimr-mumbai), [MDI Gurgaon](/colleges/mdi-gurgaon), and the IIT management departments.

For any MBA aspirant aiming for the upcoming **CAT 2026** exam, understanding candidate application statistics is not merely academic trivia—it is a critical intelligence metric. The total volume of registrations and the actual number of candidates who appear dictate the percentile calculation curve, the sectional normalization formula, and the intense cut-throat race for an IIM interview call.

Here is a deep-dive analysis of the **CAT 2026 registration status till now**, paired with official **last 5-year records of total registered vs appeared candidates**, gender distributions, category dynamics, and what these numbers mean for your preparation.

---

## CAT 2026 Total Registrations Till Now: Real-Time Status & Projections

The official registration window for **CAT 2026** commenced in the first week of August 2026 and is scheduled to close in mid-September 2026, with an expected standard 3 to 5-day correction and extension window.

### Current Registration Momentum (As of September 2026)
1. **Closing Rush Dynamics**: In every CAT cycle, nearly **45% to 50% of total candidates submit their applications during the final 10 days** leading up to the deadline.
2. **Current Estimated Registrations**: As per live application portal tracking, student counselling inquiries, and previous year registration velocity, total registration numbers for CAT 2026 have crossed the **2.75 Lakh mark** and are rapidly converging toward an estimated **3.15 to 3.30 Lakh total applicants** by the final closing date.
3. **Surge Factors**: Demand from IT, consulting, and engineering professionals seeking career upskilling, paired with booming interest from non-engineering graduates (Commerce, Arts, Science, BBA), continues to sustain high application numbers.

---

## Last 5-Year Records: CAT Registered vs Appeared Candidates (2020–2024)

To appreciate the scale of competition for CAT 2026, examining the official numbers released by the conducting IIMs over the last 5 editions provides invaluable perspective:

| CAT Exam Year | Conducting IIM | Total Registered Candidates | Total Appeared Candidates | Non-Attendees (Absent) | Attendance Percentage | YoY Registration Growth |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **CAT 2024** | **[IIM Calcutta](/colleges/iim-calcutta)** | **3,29,000** | **2,93,000** | 36,000 | **89.06%** | +0.30% |
| **CAT 2023** | **[IIM Lucknow](/colleges/iim-lucknow)** | **3,28,000** | **2,88,000** | 40,000 | **87.80%** | +28.37% |
| **CAT 2022** | **[IIM Bangalore](/colleges/iim-bangalore)** | **2,55,501** | **2,22,184** | 33,317 | **86.96%** | +11.10% |
| **CAT 2021** | **[IIM Ahmedabad](/colleges/iim-ahmedabad)** | **2,29,969** | **1,91,660** | 38,309 | **83.34%** | +0.94% |
| **CAT 2020** | **[IIM Indore](/colleges/iim-indore)** | **2,27,835** | **1,90,144** | 37,691 | **83.46%** | -6.69% |
| *CAT 2019 (Ref)* | *IIM Kozhikode* | *2,44,169* | *2,09,926* | *34,243* | *85.98%* | *+1.02%* |

### Critical Observations from the 5-Year Data:
* **The Historic Leap of 2023**: CAT 2023 recorded the steepest single-year jump in the history of the exam, rocketing by **28.37%** from 2.55 Lakh to 3.28 Lakh applicants. CAT 2024 consolidated this record with **3.29 Lakh registrations**.
* **The "Drop-Out" Cushion (11% to 17%)**: Every year without fail, between **33,000 and 40,000 registered candidates do not show up on exam day**. Your practical percentile calculation is evaluated purely against the candidates who actually sit for all three slots.
* **Attendance Peaked at 89%**: As examination centers expanded to over 170 test cities and modern multi-slot logistics stabilized, attendance rates reached an all-time high of ~89% in CAT 2024.

---

## Gender-Wise Registration & Appearance Trends Over the Last 5 Years

Diversity has been a focal point for Indian Institutes of Management. Many top IIMs now grant **2 to 5 extra composite score points** for gender diversity during the initial shortlisting stage for Personal Interviews (PI). Consequently, female applicant participation has steadily climbed.

| Exam Year | Male Registered | Female Registered | Transgender Registered | Male Appeared | Female Appeared | Transgender Appeared | Female Participation % |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **CAT 2024** | 2,10,000 | 1,19,000 | 14 | 1,86,000 | 1,07,000 | 9 | **36.17%** |
| **CAT 2023** | 2,11,000 | 1,17,000 | 12 | 1,83,000 | 1,05,000 | 8 | **35.67%** |
| **CAT 2022** | 1,66,000 | 89,501 | 6 | 1,44,420 | 77,760 | 4 | **35.03%** |
| **CAT 2021** | 1,51,770 | 78,199 | 5 | 1,26,496 | 65,160 | 4 | **34.00%** |
| **CAT 2020** | 1,52,650 | 75,180 | 5 | 1,27,396 | 62,744 | 4 | **33.00%** |

### Why Female Applications Are Surging:
1. **Academic & Gender Diversity Policies**: [IIM Rohtak](/colleges/iim-rohtak), IIM Kozhikode, and [IIM Kashipur](/colleges/iim-kashipur) have championed batches with 40% to 60%+ women cohorts.
2. **Corporate Hiring Incentives**: Top recruiters, management consultancies, and FMCG conglomerates actively seek balanced managerial cohorts during summer and final campus placements.
3. **Scholarships & Women-Focused Fellowships**: Prominent institutions provide dedicated financial aid and merit scholarships for female scholars.

---

## Category-Wise Distribution Breakdown (General vs Reserved)

The competitive pool of CAT registrations is distributed across constitutional and socio-economic reservation quotas as mandated by the Government of India for Central Educational Institutions (CEIs):

* **General (Unreserved)**: ~66.5% – 67.5% of total applicants (~2.18 Lakh candidates)
* **NC-OBC (Non-Creamy Other Backward Classes)**: ~16.8% – 17.2% of total applicants (~55,000 candidates)
* **EWS (Economically Weaker Section)**: ~4.7% – 5.1% of total applicants (~16,000 candidates)
* **SC (Scheduled Castes)**: ~8.2% – 8.6% of total applicants (~28,000 candidates)
* **ST (Scheduled Tribes)**: ~2.1% – 2.4% of total applicants (~7,500 candidates)
* **PwD (Persons with Disabilities)**: ~0.4% – 0.5% (~1,400 candidates)

> 📌 **Admission Insight**: Because percentiles are calculated nationally on a pooled basis across all candidates, scoring above the 99th percentile requires outperforming over 2,90,000 test-takers regardless of category. However, category-specific qualifying cutoffs and final composite score thresholds offer substantial advantages for reserved category students during IIM call conversions.

---

## Academic Background Shift: Engineers vs Non-Engineers

A persistent myth surrounding CAT is that it is strictly dominated by engineers. While engineering graduates still form the largest single bloc, the landscape has diversified significantly over the past 5 years:

```
CAT Academic Demographics (Recent Cycles)
┌──────────────────────────────────────────────────────────────┐
│  Engineering (B.Tech / B.E.): ~64%                           │
│  Commerce & Management (B.Com / BBA): ~21%                   │
│  Science (B.Sc / BCA): ~9%                                   │
│  Arts / Humanities / Law / Medicine / Others: ~6%            │
└──────────────────────────────────────────────────────────────┘
```

Because top IIMs (such as [IIM Ahmedabad](/colleges/iim-ahmedabad), [IIM Bangalore](/colleges/iim-bangalore), and [IIM Calcutta](/colleges/iim-calcutta)) evaluate Academic Categories (AC-1 to AC-6) and assign **academic diversity points**, non-engineering candidates can often secure interview calls at 96–98 percentile where engineers might require 99.6+ percentile.

---

## What 3 Lakh+ Registrations Mean for Your CAT 2026 Score & Percentile

When 2.90 Lakh candidates appear for an exam, the math of percentile calculations shifts. Your **percentile represents the percentage of candidates who scored equal to or less than you**.

* At 2,90,000 appeared candidates:
  * **99.9 Percentile** = Top **290 candidates** nationally
  * **99.0 Percentile** = Top **2,900 candidates** nationally
  * **95.0 Percentile** = Top **14,500 candidates** nationally
  * **90.0 Percentile** = Top **29,000 candidates** nationally

### Projected Marks vs Percentile in CAT 2026 (66 Questions / 198 Total Marks)

The CAT exam pattern has consolidated into a 2-hour test consisting of 66 questions split across **VARC (24 questions)**, **DILR (20 questions)**, and **Quantitative Aptitude (22 questions)**. Here is the estimated raw score to percentile correlation based on historical data:

| Target Percentile | Estimated Raw Score (Out of 198) | Net Correct Questions Needed | Approximate Accuracy Recommended | Target Tier / Colleges |
| :--- | :--- | :--- | :--- | :--- |
| **99.5+ %ile** | **84 – 92 Marks** | 29 – 32 Net Correct | 90%+ Accuracy | [IIM Ahmedabad](/colleges/iim-ahmedabad), [IIM Bangalore](/colleges/iim-bangalore), [IIM Calcutta](/colleges/iim-calcutta), [FMS Delhi](/colleges/fms-delhi) |
| **99.0+ %ile** | **76 – 82 Marks** | 26 – 28 Net Correct | 85%+ Accuracy | [IIM Lucknow](/colleges/iim-lucknow), IIM Kozhikode, [IIM Indore](/colleges/iim-indore), SPJIMR |
| **95.0+ %ile** | **55 – 62 Marks** | 19 – 22 Net Correct | 85%+ Accuracy | [MDI Gurgaon](/colleges/mdi-gurgaon), New IIMs (Udaipur, Trichy, Raipur, Ranchi) |
| **90.0+ %ile** | **42 – 48 Marks** | 15 – 17 Net Correct | 80%+ Accuracy | Baby IIMs, IMT Ghaziabad, [FORE School of Management](/colleges/fore-school-delhi), GIM Goa |
| **85.0+ %ile** | **34 – 39 Marks** | 12 – 14 Net Correct | 80%+ Accuracy | TAPMI, Great Lakes Chennai, BIMTECH, Lal Bahadur Shastri (LBSIM) |

---

## Top B-Schools Accepting CAT 2026: Fee vs Average Package ROI Table

When competing against 3 lakh aspirants, having a clear view of your target institutes and their returns on investment is essential:

| College Name | Total Fees (2-Year) | Avg Placement Package | ROI & Admission Eligibility / Expected Cutoff |
| :--- | :--- | :--- | :--- |
| **[FMS Delhi](/colleges/fms-delhi) (University of Delhi)** | ₹2.40 Lakhs | ₹34.10 LPA | Highest ROI in Asia · CAT 99.2+ %ile · Selection based heavily on VARC |
| **IIM Ahmedabad (PGP)** | ₹26.50 Lakhs | ₹35.22 LPA | Flagship NIRF #1 · CAT 99.5+ %ile (General) · Strong Academic Weightage |
| **IIM Bangalore (PGP)** | ₹26.00 Lakhs | ₹35.92 LPA | Premier Tech/Consulting Hub · CAT 99.3+ %ile · High Work-Ex Weightage |
| **IIM Calcutta (PGP)** | ₹25.00 Lakhs | ₹35.07 LPA | Finance Capital of India · CAT 99.5+ %ile · Quant & DILR friendly |
| **[SPJIMR Mumbai](/colleges/spjimr-mumbai) (PGDM)** | ₹24.00 Lakhs | ₹33.00 LPA | Top Private Institute · Profile-based calls at CAT 85+ %ile / Score-based at 98+ %ile |
| **[MDI Gurgaon](/colleges/mdi-gurgaon) (PGDM)** | ₹26.00 Lakhs | ₹25.50 LPA | Prime Delhi-NCR Location · CAT 95.0+ %ile · Strong Corporate Network |
| **[IIT Bombay](/colleges/iit-bombay) (SJMSOM)** | ₹14.50 Lakhs | ₹28.88 LPA | Top Engineering ROI · CAT 98.5+ %ile · Open to 4-year degree holders |
| **IIT Delhi (DMS)** | ₹12.00 Lakhs | ₹25.82 LPA | Exceptional Corporate ROI · CAT 98.0+ %ile · Open to multiple streams |

---

## Strategic Preparation Advice for the Final Lap of CAT 2026

With total applications tracking in the 3.2 Lakh bracket, casual preparation will not suffice. To stand among the top 1% of test-takers:

1. **Prioritize Question Selection Over Attempts**: You do not need to attempt all 66 questions. Scoring 78 marks out of 198 (around 40% of the maximum marks) secures a **99th percentile**. Accuracy and disciplined question elimination are your greatest assets.
2. **Master the 40-Minute Sectional Clock**: CAT does not allow cross-sectional time switching. You must cultivate high stamina to perform at peak cognitive clarity for 40 minutes in VARC, followed immediately by 40 minutes of intense DILR puzzle solving, and culminating in 40 minutes of Quantitative Aptitude.
3. **Simulate Real Testing Environments with Full-Length Mocks**: Take at least 20 to 25 proctored mock tests in the exact 8:30 AM – 10:30 AM, 12:30 PM – 2:30 PM, or 4:30 PM – 6:30 PM exam slots to synchronize your body clock.

---

## Related Reading & Essential Guides

To strengthen your 2026–2027 admission roadmap, explore our expert resources:
* **[10 Tips to Crack CAT Exam 2026: Complete Strategy Guide](/blog/10-tips-to-crack-cat-exam-2026)**
* **[All About CAT Exam 2026: Syllabus, Pattern, Registration & IIM Cutoffs](/blog/all-about-cat-exam)**
* **[All IIM Cut-Offs 2026–2028: Category-Wise Admission Criteria](/blog/all-iim-cut-off-2026-28-admission-mba-pgdm)**
* **[Top IIM Colleges Placement Report & Fee Structures](/blog/all-about-iim-colleges-placements-fees-selection-2026)**
* **[MBA & PGDM Admission 2027–2029 Complete Guide](/blog/mba-pgdm-admission-2027-2029-complete-guide)**

---

> 🎯 **Measure Your Readiness Against 3 Lakh Candidates**
> 
> Don't leave your IIM dream to chance. Assess your real-time percentile and benchmark your sectional preparation today.
> 
> [👉 Take the Free AI-Powered CAT 2026 Mock Test Now](/tools/cat-mock-test)

---

### 🚀 Boost Your Preparation

Looking for more resources? **[Explore Our Premium MBA Mock Test Series 2026](/mock-tests)** to get real-time exam experience and detailed performance analytics.

---
