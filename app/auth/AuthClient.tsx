...
export default function AuthClient() {
  ...
  // Initialize and render Google Login/Sign-up buttons
  useEffect(() => {
    if (typeof window !== "undefined") {
      const initializeGoogle = () => {
        if ((window as any).google) {
          (window as any).google.accounts.id.initialize({
            client_id: process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || "1028717834785-gsuidsp1t01dqu5o3uuhd08v1a0rpn1k.apps.googleusercontent.com",
              ...
              try {
                ...
                setTimeout(() => {
                  ...
                  setLoggedInUser({
                    name: profile.name,
                    email: profile.email,
                    phone: "9999999999", // Google doesn't share phone numbers
                    picture: profile.picture
                  });
                  setSuccess(`Welcome ${profile.name}! Authenticated via Google.`);
                }, 500);
              } catch (err) {
                setError("Failed to decode Google user credentials.");
              }
            }
          });

          // Render login button if element exists
          ...
          // Render signup button if element exists
          const signupBtn = document.getElementById("google-signin-signup-btn");
          if (signupBtn) {
            (window as any).google.accounts.id.renderButton(signupBtn, {
              theme: "outline",
              size: "large",
              text: "signup_with",
              shape: "rectangular",
              logo_alignment: "left",
              width: signupBtn.parentElement?.clientWidth || 300
            });
          }
        }
      };

      // Poll for Google script presence
              ...
              {showLoginOtp ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="w-full flex justify-center pt-2">
                    <div id="google-signin-login-btn" className="w-full"></div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              {showSignupOtp ? (
                ...
                <form onSubmit={handleSendSignupOtp} className="space-y-5">
                  ...
                  <div className="w-full flex justify-center pt-2">
                    <div id="google-signin-signup-btn" className="w-full"></div>
                  </div>
                </form>
              )}
            </div>
